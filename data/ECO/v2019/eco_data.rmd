...
for(f in grep(".*csv", list.files(dir_rawdata), value = TRUE)){
...
eco_data_bhirgns <- read_csv(file.path(dir_prep, "supplement", "lookup_tabs", "rgns_complete.csv")) %>% 
  ...
  full_join(eco_data_combined, by = "country") %>% 
  ...
  mutate(value_added_year = ifelse(is.na(annualgrowthrate), NA, value_added_year))
...
write_csv(
  select(eco_data_bhirgns, region_id, sector, year, gva_yearly_estim = value_added_year),
