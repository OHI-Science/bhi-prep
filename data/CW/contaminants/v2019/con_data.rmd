...
<!-- note: the following incorporates, along with preliminary contaminants_prep.Rmd sections, the following BHI1.0 scripts: -->
<!-- raw_pcb_data_prep.R, raw_dioxin_data_prep.R, raw_pfos_data_prep.R -->
<!-- station_attribute_prep.R -->
<!-- congener_description.R -->
...
```{r rename fields/vars - PCBs sediment}
...
**PCBs**
The main objective of the first code chunk below is to clean the PCBs-biota data set so it contains only values based on wet weight.
...
```{r standardize units - PCBs biota}
...
) %>% unlist() # pcb_bio %>% filter(is.na(sub_samp_ref))
...
pcb_bio_final <- pcb_bio_final %>% 
  ...
  mutate(
    ...
    value = ifelse(
      ...
      ## dreissena polymorpha, perca fluviatilis, mytilus edulis, abramis brama
      ifelse(
...
ggplot(pcb_bio_final) + 
  geom_point(aes(date, value, colour = latitude), alpha = 0.3) +
...
```{r standardize units - PCBs sediment}
...
```{r species samples distrubutions -- PCBS}
map_pcb_bio <- ggplot(spread(pcb_bio_final, "something")) + 
  geom_map()
map_pcb_sed <- ggplot() +
  geom_map()
```

```{r sampling densities by year and parameter}
