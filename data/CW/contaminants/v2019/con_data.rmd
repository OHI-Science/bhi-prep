...
biota_standardize <- function(biotadf, contam_param, rm_vars = NULL){
  ...
  if(var(chk) != 0){
    message(sprintf(
      "bbio duplicates dataframe dims vary with ID variables, investigate in grpvars: %s",
  ...
  message(paste(
    "minimum group ID variables for later rejoining uniquely with congener data:\n", 
    paste(chk, collapse = ", ")
  ))
  ...
  df_conc <- bind_rows(
    ...
    df_conc %>% 
      ...
      summarize(
        ...
        ## how best to handle quant lim?
        ## what if duplicates average to over/under lim...
        quant_lim = any(quant_lim) 
      ) %>% 
      ungroup(),
  ...
  df_final <- df_conc_wide %>% 
    mutate(sub_samp_id = as.numeric(sub_samp_id)) %>% 
