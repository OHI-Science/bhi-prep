...
#### 2.1.5 PCB Concentration Indicator
...
standardize_con_data <- function(data_renamed, matrix_biota = TRUE, contam_param, rm_vars = NULL){
  ...
  for(i in 1:6){
    bbio_duplicates <- df_bbio %>% 
      group_by(!!!syms(c(grpvars[1:i], grpvars[7:length(grpvars)]))) %>%
      summarise(n = n()) %>% 
      filter(n > 1) %>% 
      ungroup()
  ...
  if(var(chk) != 0){
    message(sprintf(
      ...
      paste(grpvars[which(chk == quantile(chk, 0.1))], collapse = ", ")
    ))
  }
  ...
  if(!chk_lims_ids){warning("detect and quant limit variables not uniquely identified by grouping variables")}
  ...
  if(matrix_biota){
    ...
    chk_id <- df_conc_wide %>%
      ...
      filter(n > 1) %>% 
      ungroup()
    chk_id <- unique(chk_id[["sub_samp_ref"]])
    df_conc_wide <- df_conc_wide %>%
      filter(!(sub_samp_ref %in% chk_id & basis_determination == "lipid weight" & !is.na(qflag)))
  }
  ...
  df_final <- df_conc_wide %>% 
    ...
    ## gathering into long format after having spread by congener will give lots of NAs
    ## these'll be filtered out in the last line before result is returned
    tidyr::pivot_longer(
...
#### 2.3.1 Congeners by Country {.tabset .tabset-fade}

##### Plotting Data

```{r congeners barplots dataframe, echo = TRUE, results = "show"}
...
```{r congeners monthly by country, results = "show", message = FALSE, echo = TRUE, fig.width = 9.5, fig.height = 11}
## monthly plots
plotly::ggplotly(
...
```{r congeners yearly by country, results = "show", message = FALSE, echo = TRUE, fig.width = 9.5, fig.height = 11}
## yearly plots
plotly::ggplotly(
...
make_mapdf <- function(x, y){
  cbind(
    get(x) %>% 
      ...
      mutate(
        mean_all = mean(mean_val),
        sd_all = ifelse(is.na(sd(mean_val)), 1, sd(mean_val) +1)
      ) %>%
      tidyr::pivot_longer(
        cols = c(sd_all, mean_all), 
...
mapdf_sediment <- do.call(
  ...
  lapply(
    ...
    function(x) make_mapdf(x, 2014:2019)
  )
)
...
```{r contaminants in biota vs sediment histograms, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 10}
...
boxdf <- boxdf %>% 
  filter(metric == "mean_all") %>% 
  group_by(source) %>% 
  mutate(quant9 = quantile(mean_val, 0.9)) %>% 
  ungroup() %>% 
  filter(mean_val < quant9)

ggplot(filter(boxdf, source != "pfos"), aes(x = variable, y = mean_val, color = matrix)) + 
