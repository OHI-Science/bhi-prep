...
## PCBs data from ICES
pcb_rawdata_bio <- read_csv(
  file.path(dir_rawdata, "ContaminantsBiota_PCBs", "ContaminantsBiota_PCBs.csv"),
  col_types = cols(SUBNO = col_character(), DETLI = col_number(), LMQNT = col_number(), BULKID = col_integer())
)
pcb_rawdata_sed <- read_csv(
  file.path(dir_rawdata, "ContaminantsSediment_PCBs", "ContaminantsSediment_PCBs.csv"),
  col_types = cols(SUBNO = col_character(), DETLI = col_number(), LMQNT = col_number(), BULKID = col_integer())
)
...
pfos_rawdata_bio <- read_csv(
  file.path(dir_rawdata, "ContaminantsBiota_PFOS", "ContaminantsBiota_PFOS.csv"),
  col_types = cols(SUBNO = col_character(), DETLI = col_number(), LMQNT = col_number(), BULKID = col_integer())
)

## Dioxin data from ICES
dioxin_rawdata_bio <- read_csv(
  file.path(dir_rawdata, "ContaminantsBiota_Dioxins", "ContaminantsBiota_Dioxins.csv"),
  col_types = cols(SUBNO = col_character())
)
dioxin_rawdata_sed <- read_csv(
  file.path(dir_rawdata, "ContaminantsSediment_Dioxins", "ContaminantsSediment_Dioxins.csv"),
  col_types = cols(SUBNO = col_character(), DETLI = col_number(), LMQNT = col_number(), BULKID = col_integer())
)
```
...
biota_standardize <- function(biotadf, contam_param, rm_vars = NULL){
  ...
  grpvars <- c(
    "sub_samp_id", "samp_ref", "sub_samp_ref", "date", "variable",
  ...
  df_conc <- df %>%
    ...
    select(
      station, latitude, longitude, date, year, 
      sub_samp_ref, sub_samp_id, samp_ref,
      species, matrix_analyzed, basis_determination,
  ...
  chk_conc_wide_id <- df_conc_wide %>%
    ...
    filter(n > 1) %>% 
    ungroup()
  chk_conc_wide_id <- unique(chk_conc_wide_id$sub_samp_ref)
  
  df_conc_wide <- df_conc_wide %>%
    filter(!(sub_samp_ref %in% chk_conc_wide_id & basis_determination == "lipid weight"))
  
  ## check one row for every unique sub_samp_ref
  chk_conc_sub_samp <- length(unique(df_conc$sub_samp_ref)) == nrow(distinct(df_conc_wide))
  ...
  message("STEP 6: rejoining with meta information, finalizing\n")
  df_final <- df %>% 
    filter(str_detect(param_group, pattern = contam_param)) %>%
    select(
...
write_csv(
  pcb_bio_lst$df_final, 
  here::here(dir_interm, "pcb_biota_cleaned.csv")
)
...
write_csv(
  dioxin_bio_lst$df_final, 
  here::here(dir_interm, "dioxin_biota_cleaned.csv")
)

## wrangle pfos dataset
pfos_bio_lst <- biota_standardize(
  pfos_bio, 
  contam_param = "O-FL"
)
write_csv(
  pfos_bio_lst$df_final,
  here::here(dir_interm, "pfos_biota_cleaned.csv")
)
```
...
mapdf_sediment <- do.call(rbind, lapply(list("pcb_sed", "dioxin_sed"), function(x) make_mapdf(x, 2013)))
