...
## Create an additional lookup table from the dataset used in the BHI 1.0 ('ices_secchi')
basin_additional_lookup_table <- ices_secchi %>%
  select(-X1, -secchi, -Month, -Year, -Date, -Station)
  
readr::write_csv(
  basin_additional_lookup_table, 
  file.path(dir_lookup, "ices_basin_add_lookup_table.csv")
)

## Create an additional lookup table from the dataset used in the BHI 1.0 ('smhi_secchi')
basin_add_lookup_table_smhi <- smhi_secchi %>%
  select(-X1, -value, -unit, -Month, -Year, -Date) %>% 
  rename(Station = Stationsnamn, Visit_event_id = Provtagningstillfaelle.id)
...
# Merge ices secchi data with helcom coastal codes
all_ices_secchi <- basin_add_lookup_table %>% 
  left_join(ices_unique, by = c("Latitude", "Longitude", "Cruise"))  

# Check for duplicates from the merged dataset
all_ices_duplicates <- all_ices_secchi %>%
  group_by(Date, Latitude, Longitude, Assessment_unit, HELCOM_COASTAL_CODE, HELCOM_ID, 
           BHI_ID, Cruise, Station, secchi, Year, Month, bottom_m) %>%
  summarise(n = n()) %>%
  filter(n > 1) %>%
  ungroup() 

all_ices_unique <- all_ices_secchi %>% 
  distinct(Date, Latitude, Longitude, Assessment_unit, HELCOM_COASTAL_CODE, HELCOM_ID, 
           BHI_ID, Cruise, Station, secchi, Year, Month, bottom_m) %>% 
    mutate(supplier = 'ices') 

## Which ices data have BHI_ID of NA?
ices.na <- all_ices_unique %>%
  filter(is.na(BHI_ID))
dim(ices.na) # 497 14
...
all_secchi_smhi <- all_secchi_smhi %>% 
  mutate(Value = as.numeric(as.character(Value)), Parameter = as.character(Parameter))

# Check for duplicates in smhi
smhi_duplicates <- all_secchi_smhi %>%
    group_by(Station.name, Value, Unit, Sample.latitude..DD., 
...
# Only extract unique observations
smhi_unique <- all_secchi_smhi %>% 
  distinct(Station.name, Value, Unit, Sample.latitude..DD., 
           Sample.longitude..DD., Sampling.date, Year, Shark.sample.id.MD5, Station.ID..VISS., 
           HELCOM.OSPAR.area, Station.water.depth, Visit.event.identifier) %>% 
  rename(secchi = Value, Date = Sampling.date, Visit_event_id = Visit.event.identifier,
  ...
  select(-Unit, -Shark.sample.id.MD5, -HELCOM.OSPAR.area) %>% 
  mutate(Date =  as.Date(Date), Month = format(Date, "%m"),
         Month = str_replace(Month, "^0+" ,""),
         Year = as.character(Year)) 

# Merge smhi secchi data with helcom coastal codes
all_smhi_secchi11 <- basin_add_lookup_table %>% 
  select(-Cruise) %>% 
  full_join(smhi_unique, by = c("Latitude", "Longitude"))  

all_smhi_secchi22 <- basin_add_lookup_table_smhi %>% 
  select(-Visit_event_id, -Station) %>% 
  full_join(smhi_unique, by = c("Latitude", "Longitude"))

# Check for duplicates from the merged dataset (1)
all_smhi_duplicates11 <- all_smhi_secchi11 %>%
  group_by(Date, Latitude, Longitude, Assessment_unit, HELCOM_COASTAL_CODE, HELCOM_ID, 
           BHI_ID, Cruise, Station, secchi, Year, Month, bottom_m) %>%
  summarise(n = n()) %>%
  filter(n > 1) %>%
...
all_smhi_unique11 <- all_smhi_secchi11 %>% 
  distinct(Date, Latitude, Longitude, HELCOM_COASTAL_CODE, HELCOM_ID, 
           BHI_ID, Visit_event_id = Cruise, Station, secchi, Year, Month, bottom_m) %>% 
  mutate(supplier = 'smhi')

# Check for duplicates from the merged dataset (2)
all_smhi_duplicates22 <- all_smhi_secchi22 %>%
  group_by(Date, Latitude, Longitude, HELCOM_COASTAL_CODE, HELCOM_ID, 
           BHI_ID, Visit_event_id, Station, secchi, Year, Month, bottom_m) %>%
  summarise(n = n()) %>%
  filter(n > 1) %>%
  ungroup() 

all_smhi_unique22 <- all_smhi_secchi22 %>% 
  distinct(Date, Latitude, Longitude, HELCOM_COASTAL_CODE, HELCOM_ID, 
           BHI_ID, Visit_event_id, Station, secchi, Year, Month, bottom_m) %>% 
  mutate(supplier = 'smhi')

all_smhi_unique3 <- rbind(
  all_smhi_secchi11, all_smhi_secchi22
)
##### Check Extra duplicates 
# Check for duplicates from the merged dataset (2)
all_smhi_duplicates3 <- smhi_secchi %>%
  group_by(X1, Date, Latitude, Longitude, HELCOM_COASTAL_CODE, HELCOM_ID, 
           BHI_ID, Provtagningstillfaelle.id, Stationsnamn, value) %>%
  summarise(n = n()) %>%
  filter(n > 1) %>%
  ungroup() 
...
## is na smhi
smhi.na2 <- all_smhi_unique22 %>%
  filter(is.na(BHI_ID))
dim(smhi.na1) # 49882  14

smhi.na.loc2 <- smhi.na2 %>% 
  dplyr::select(Latitude, Longitude) %>% 
  distinct() ## unique locations
dim(smhi.na.loc1) # 3762   2
