...
lapply(
  ...
  function(x){
    tmp <- read.delim(file.path(dir_rawdata, "OxygenDebt_ICES", x), sep = ",")
  ...
  cbind(
    get(x) %>% 
      ...
      mutate(
        ...
        Oxygen_Original = Oxygen,
        Oxygen = as.numeric(Oxygen),
        Depth = as.numeric(Depth)
      ),
    Country = stringr::str_remove_all(x, "ox|_|[0-9]")
  )
}))
...
ox_merged_w_rgns <- join_rgns_info(
  dataset = ox_merged,
  rgn_shps_loc = file.path(dir_B, "Shapefiles")
)
...
# ctddata_to_merge <- ls(pattern = "ctd_[a-z].*ctd$")
# 
# ctd_merged <- do.call(rbind, lapply(oxdata_to_merge, function(x){
#   cbind(
#     get(x) %>% 
#       rename(
#         Oxygen = DOXY..ml.l.,
...
#         Depth = as.numeric(Depth)
#       ),
#     Country = stringr::str_remove_all(x, "ctd|_|[0-9]")
#   )
# }))
# 
...
readr::write_csv(
  select(ox_merged_w_rgns, -BHI_ID, -Country, -Subbasin), 
  file.path(dir_interm, "ox_merged_rawdata.csv")
)
# readr::write_csv(
#   select(ctd_merged_w_rgns, -BHI_ID, -Country, -Subbasin), 
