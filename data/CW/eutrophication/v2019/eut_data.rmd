...
ices_unique <- ices %>% 
  distinct(
    Cruise, Station, Date, Latitude, Longitude, BHI_ID, Assessment_unit,
    HELCOM_COASTAL_CODE, HELCOM_ID, Year, Month, secchi, bottom_m
  ) %>% 
  mutate(supplier = "ices") # 104773

## Which ices data have BHI_ID of NA?
ices.na <- ices_unique %>%
  filter(is.na(BHI_ID))
dim(ices.na) # 64450 14
...
# Only extract unique observations
smhi_unique <- smhi %>% 
  distinct(Cruise, Station, Date, Latitude, Longitude, BHI_ID, Assessment_unit,
           HELCOM_COASTAL_CODE, HELCOM_ID, Year, Month, secchi, bottom_m) %>% 
  mutate(supplier = 'smhi') # 51448

## is na smhi
smhi.na <- smhi_unique %>%
  filter(is.na(BHI_ID))
dim(smhi.na) # 49123  14
...
secchi <- bind_rows(ices_unique, smhi_unique) %>% 
  distinct() %>% # 231545 tot observations 
...
```{r merge two finish chla datasets, results = "show", message = FALSE, echo = TRUE, fig.width = 9.5, fig.height = 4.5}
## Data overview
