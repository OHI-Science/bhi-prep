...
Each dataset has been extracted by country and Denmark split into five (5) because too big otherwise.
...
Each dataset has been extracted by country, Denmark split into three (3) and Germany into five (5) because too big otherewise. No CTD found for Lithuania.
...
```{r using get_nest_data function to download summer chlorophyll a data, eval = FALSE}
get_nest_data(
  date_range = c(20050101, 20191231), 
  months = 5:10, 
  param_codes = c("CHL")
...
Secchi data will be first assessed at this level and then assigned to BHI region. EEZ divisions may result in some BHI regions that have no data but they are physically the same basin as a BHI region with data. Also, the dataset `HELCOM Subbasins with coastal and offshore division 2018` will be used for Secchi as well as Chlorophyll a, DIN and DIP, to assign 'coastal' and 'offshore' locations.
...
- This should result in a similar dataset (see map) used as Fleming-Lehtinen and Laamanen 2012  
...
```{r chla raw datasets, results = "show", echo = TRUE, fig.height = 5, fig.width = 9.5}
smhi_secchi_map <- basemap +
  geom_sf(
    size = 0.8, alpha = 0.5,
    mapping = aes(color = HELCOM.OSPAR.area), 
    data = sf::st_as_sf(
      filter(secchi_smhi_correctcols, HELCOM.OSPAR.area %in% c("HELCOM", "OSPAR/HELCOM")), 
      coords = c("Sample.longitude..DD.", "Sample.latitude..DD."), 
      crs = 4326
    )
  ) +
  labs(color = "Assessement Area") +
...
smhi_chla_map <- basemap +
  geom_sf(
    ...
    mapping = aes(color = HELCOM.OSPAR.area), 
    data = sf::st_as_sf(
      filter(chla_smhi_correctcols, HELCOM.OSPAR.area %in% c("HELCOM", "OSPAR/HELCOM")), 
      coords = c("Sample.longitude..DD.", "Sample.latitude..DD."), 
      crs = 4326
    )
  ) +
...
gridExtra::grid.arrange(smhi_secchi_map, smhi_chla_map, nrow = 1)
```
<br>

#### 2.3.2 Oxygen data

```{r map showing oxygen datasets, results = "show", warning = FALSE, echo = TRUE, fig.height = 8, fig.width = 10}
dir_interm <- here::here("data", "CW", "eutrophication", version_year, "intermediate")
...
oxmap <- basemap + geom_sf(data = plotsf, size = 0.8, alpha = 0.02)

## plot by subbasins
plotdf <- ox_merged_w_rgns %>% 
  filter(!is.na(Assessment_Unit)) %>% 
  # group_by(Assessment_Unit) %>% 
  # sample_n(6000) %>% 
  mutate(Date = as.Date(paste(Year, Month, Day, sep = "-")))

oxplot <- ggplot(plotdf, aes(Oxygen, Depth, color = Year)) + 
  geom_point(alpha = 0.02, size = 0.3, show.legend = FALSE) +
  facet_wrap(~Assessment_Unit, scales = "free_y")  +
  labs(x = "Dissolved Oxygen (ml/l)", y = "Depth (m)") +
  scale_y_reverse() +
  scale_color_viridis_c(option = "plasma")

## timeseries plot
oxtsplot <- ggplot(plotdf, aes(Date, Oxygen, color = Year)) + 
  geom_point(size = 0.2, alpha = 0.02, show.legend = FALSE) +
  facet_wrap(~Assessment_Unit, ncol = 3) +
  labs(x = NULL, y = "Dissolved Oxygen (ml/l)") +
  scale_color_viridis_c(option = "plasma")

gridExtra::grid.arrange(oxmap, oxplot, nrow = 1, widths = c(1, 1.2))
```
<br/>
...
pal <- colorRampPalette(RColorBrewer::brewer.pal(8, "Set2"))(12)
