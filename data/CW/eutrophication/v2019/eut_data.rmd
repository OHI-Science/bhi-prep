...
secchi_target <- eut_thresholds %>% 
  dplyr::select(Subbasin = basin, secchi_target = summer_secchi) %>%
  mutate(Subbasin = str_replace_all(Subbasin, "_", " "),
         secchi_target = as.factor(secchi_target))   
...
**Oxygen**
```{r read raw datasets, message = FALSE, warning = FALSE, echo = TRUE, results = "hide"}
oxden1 <- read.delim(file.path(dir_rawdata, "OxygenDebt_ICES", "denmarkox", "denmark2000-2001ox.csv"), sep = ",")
oxden2 <- read.delim(file.path(dir_rawdata, "OxygenDebt_ICES", "denmarkox", "denmark2002-2004ox.csv"), sep = ",")
oxden3 <- read.delim(file.path(dir_rawdata, "OxygenDebt_ICES", "denmarkox", "denmark2005-2009ox.csv"), sep = ",")
oxden4 <- read.delim(file.path(dir_rawdata, "OxygenDebt_ICES", "denmarkox", "denmark2010-2014ox.csv"), sep = ",")
oxden5 <- read.delim(file.path(dir_rawdata, "OxygenDebt_ICES", "denmarkox", "denmark2015-2020ox.csv"), sep = ",")
...
oxger1 <- read.delim(file.path(dir_rawdata, "OxygenDebt_ICES", "germanyox", "germany2000-2009ox.csv"), sep = ",")
oxger2 <- read.delim(file.path(dir_rawdata, "OxygenDebt_ICES", "germanyox", "germany2010-2020ox.csv"), sep = ",")
...
ggplot(secchi_rgns_shp) + 
  geom_point(aes(month, secchi, colour = supplier)) +
  facet_wrap(~BHI_ID, scales ="free_y")

ggplot(secchi_rgns_shp) + 
  geom_point(aes(year ,secchi, colour = supplier)) +
  facet_wrap(~BHI_ID)
...
oxswe_duplicates <- oxswe %>%
  # oxden2_duplicates <- oxden2 %>%
  # oxden3_duplicates <- oxden3 %>%
  # oxden4_duplicates <- oxden4 %>%
  # oxden5_duplicates <- oxden5 %>%
  # oxest_duplicates <- oxest %>%
  # oxfin_duplicates <- oxfin %>%
  # oxger_duplicates <- oxger %>%
  # oxlat_duplicates <- oxlat %>%
  # oxlit_duplicates <- oxlit %>%
  # oxpol_duplicates <- oxpol %>%
  # oxswe_duplicates <- oxswe %>%
...
oxswe_unique <- oxswe %>% 
  ...
  # oxden3_unique <- oxden3 %>%
  # oxden4_unique <- oxden4 %>%
  # oxden5_unique <- oxden5 %>%
  # oxest_unique <- oxest %>%
  # oxfin_unique <- oxfin %>%
  # oxger_unique <- oxger %>%
  # oxlat_unique <- oxlat %>%
...
oxygen <- bind_rows(
  oxden1_unique %>% 
    rename(oxygen_ml_l = DOXY..ml.l.,  date = yyyy.mm.ddThh.mm, 
           lat = Latitude..degrees_north., lon = Longitude..degrees_east., 
           cruise = Cruise, station = Station, temp = TEMP..deg.C., sal_psu = PSAL..psu., 
           press_db = PRES..db., bottom_m = Bot..Depth..m.) %>%
    mutate(date =  as.Date(date), year = format(date, "%Y"), 
           date =  as.Date(date), month = format(date, "%m"),
  ...
  oxden2_unique %>% 
    ...
    mutate(date =  as.Date(date), year = format(date, "%Y"), 
           date =  as.Date(date), month = format(date, "%m"),
           oxygen_ml_l = str_replace(oxygen_ml_l, "<", ""),
...
readr::write_csv(
  oxygen, 
  here::here("data", "CW", "eutrophication", version_year, "intermediate", "ox_merged_rawdata.csv")
)
...
oxyg_rgns_shp <- join_rgns_info(
  oxygen, helcomID_col = "helcom_id", country_col = "country", 
                           latlon_vars = c("^lat", "^lon"), return_spatial = TRUE, 
                           rgn_shps_loc = file.path(dir_B, "Spatial"), buffer_shp = NULL)

## Merge the oxygen target (does not work)!! 
secchi_rgns_shp <- secchi_rgns_shp %>% 
  left_join(secchi_target, by = "Subbasin")
