...
These sub-basins are the relevant physical units.  

Secchi data will be first assessed at this level and then assigned to BHI region. EEZ divisions may result in some BHI regions that have no data but they are physically the same basin as a BHI region with data. The dataset `HELCOM Subbasins with coastal and offshore division 2018` will be used for Secchi as well as Chlorophyll a, DIN and DIP, to assign 'coastal' and 'offshore' locations.
...
##### 2.1.5.1 Coastal Chlorophyll a Reference Values {-}
...
##### 2.1.5.2 Coastal DIN Reference Values {-}

The coastal threshold values for Chlorophyll a are presented in the [HELCOM Chlorophyll a core indicator](https://helcom.fi/media/core%20indicators/Dissolved-inorganic-nitrogen-DIN-HELCOM-core-indicator-2018.pdf) (p.7-8, Results Table 2).
...
##### 2.1.5.2 Coastal DIP Reference Values {-}

The coastal threshold values for Chlorophyll a are presented in the [HELCOM Chlorophyll a core indicator](https://helcom.fi/media/core%20indicators/Dissolved-inorganic-phosphorus-DIP-HELCOM-core-indicator-2018.pdf) (p.6-7, Results Table 2).
...
- Non-coastal (offshore) data are flagged with the code "0" under the column *"HELCOM_COASTAL_CODE"*  
- HOLAS basin shape files with coastal and non-coastal areas were overlaid with the Chl _a_ sampling locations, all locations were flagged with a code indicating coastal or offshore. 
...
```{r chla raw datasets, results = "show", echo = TRUE, fig.height = 4.8, fig.width = 9}
...
gridExtra::grid.arrange(smhi_secchi_map, smhi_chla_map, nrow = 1, widths = c(1, 1.3))
```
<br>
...
```{r map showing oxygen datasets, results = "show", message = FALSE, warning = FALSE, echo = TRUE, fig.height = 4.5, fig.width = 4}
dir_interm <- here::here("data", "CW", "eutrophication", version_year, "intermediate")
...
gridExtra::grid.arrange(oxmap, ctdmap, nrow = 1, widths = c(1.2, 1))
