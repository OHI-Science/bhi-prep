...
ices_unique <- secchi_raw_ices %>% 
  distinct(yyyy.mm.ddThh.mm, Latitude..degrees_north., Longitude..degrees_east., 
           Secchi.Depth..m., Bot..Depth..m., Station, Cruise)

# Check if there are no more duplicates
ices_duplicates2 <- ices_unique %>%
    group_by(Station, Cruise, Secchi.Depth..m., Latitude..degrees_north., 
             Longitude..degrees_east.,yyyy.mm.ddThh.mm) %>%
    summarise(n = n()) %>%
    filter(n > 1) %>%
...
smhi_unique <- secchi_raw_smhi %>% 
  distinct(Station.name, Value, Sample.latitude..DD., 
           ...
           HELCOM.OSPAR.area, Station.water.depth)
...
#### Add "supplier" column to differentiate ICES from SMHI data (TO-DO)!!!
str(secchi_unique)
