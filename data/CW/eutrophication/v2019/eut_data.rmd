...
**Baltic Nest datasets Chlorophyll a, and Nutrients**
...
Retrieve Baltic Nest Nutrients datasets:
...
knitr::kable(tab, caption = "Source: [ICES database](https://ocean.ices.dk/HydChem/HydChem.aspx?plot=yes) <br/> Downloaded 30 April 2020 by Andrea De Cervo")

```
*The maximum number of stations per download is 20000*
<br/>

**Baltic Nest Nutrients**
...
Also, the dataset 'HELCOM Subbasins with coastal and offshore division 2018' will be used for Secchi to assign 'coastal' and 'offshore' locations.
...
#### 2.2.2 Read data for Initial Cleaning and Merging {-}
...
## need to find a way to open and properly read the txt file
# secchi_raw_smhi2 <- read.csv(file.path(dir_rawdata, "SecchiDepth_SMHI", "sharkweb_data_secchi.txt"), sep = "\t")
...
## need to find a way to open and properly read the txt file
# chla_raw_smhi2 <- read.csv(file.path(dir_rawdata, "sharkweb_data_chla.txt"), sep = "\t")

## read Nutrients raw dataset
nutrients_rawdata <- read.delim(
  file.path(dir_rawdata, "NestData", "nestdata_winter_nutrients.csv"), 
  sep = ",", 
...
**ICES chla data**
```{r read raw datasets, message = FALSE, warning = FALSE, echo = TRUE, results = "hide"}
## this will read in the chla datasets from the Chla_ICES directory
## in the same step, this will check for duplicates and then keep only the distinct observations
chla_den1 <- read.delim(file.path(dir_rawdata, "Chla_ICES", "denmark_chla", "denmark_chla_2000-2005.csv"), sep = ",")
chla_den2 <- read.delim(file.path(dir_rawdata, "Chla_ICES", "denmark_chla", "denmark_chla_2006-2017.csv"), sep = ",")
chla_den3 <- read.delim(file.path(dir_rawdata, "Chla_ICES", "denmark_chla", "denmark_chla_2018-2020.csv"), sep = ",")
chla_est <- read.delim(file.path(dir_rawdata, "Chla_ICES", "estonia_chla", "estonia_chla_2000-2020.csv"), sep = ",")
chla_fin <- read.delim(file.path(dir_rawdata, "Chla_ICES", "finland_chla", "finland_chla_2000-2020.csv"), sep = ",")
chla_ger <- read.delim(file.path(dir_rawdata, "Chla_ICES", "germany_chla", "germany_chla_2000-2020.csv"), sep = ",")
chla_lat <- read.delim(file.path(dir_rawdata, "Chla_ICES", "latvia_chla", "latvia_chla_2000-2020.csv"), sep = ",")
chla_lit <- read.delim(file.path(dir_rawdata, "Chla_ICES", "lithuania_chla", "lithuania_chla_2000-2020.csv"), sep = ",")
chla_pol <- read.delim(file.path(dir_rawdata, "Chla_ICES", "poland_chla", "poland_chla_2000-2020.csv"), sep = ",")
chla_rus <- read.delim(file.path(dir_rawdata, "Chla_ICES", "russia_chla", "russia_chla_2000-2020.csv"), sep = ",")
chla_swe <- read.delim(file.path(dir_rawdata, "Chla_ICES", "sweden_chla", "sweden_chla_2000-2020.csv"), sep = ",")
```
<br/>

#### 2.2.3 Secchi Datasets {-}
...
```{r cleaning raw ICES secchi data, results = "hide"}
...
```{r which ices data have bhi id of NA, eval = FALSE}
ices.na <- ices_unique %>%
  filter(is.na(BHI_ID))
dim(ices.na) # 64437
...
```{r cleaning raw SMHI secchi data, results = "hide"}
### SMHI data overview
...
smhi_duplicates <- smhi %>%
  group_by(Cruise, Station, Date, Latitude, Longitude, secchi, 
           bottom_m, Year, Month) %>%
  summarise(n = n()) %>%
  filter(n > 1) %>%
  ungroup() 
...
secchi <- bind_rows(smhi_unique, ices_unique) %>% 
...
chla_duplicates <- chla_nest %>%
    group_by(
      cruise, date, year, month, Latitude,
...
chla_nest_unique <- chla_nest %>% 
  distinct(cruise, date, year, month, Latitude, Longitude, chla_ug_l, 
           depth_m) %>% 
  mutate(supplier = 'nest')
# 44071
```

```{r which ices data have BHI_ID of NA, eval = FALSE}
chla.na <- chla_unique %>%
  filter(is.na(BHI_ID))
dim(chla.na) # 21591 
...
chla_raw_ices <- bind_rows(
  ...
  mutate(
    ...
    Bot_Depth = as.numeric(Bot_Depth),
...
# Save raw and merged dataset (bhi-prep)
# readr::write_csv(
#  chla_ices, 
#  here::here("data", "CW", "eutrophication", version_year, "intermediate", "chla_ices.csv")
#)

#### Look for duplicate data 
# Check for duplicates in ices
ices_duplicates <- chla_raw_ices %>%
  group_by(cruise = Cruise, date = Date, Latitude, Longitude, chla_ug_l = Chla_ug_l, 
           depth_m = Bot_Depth, year = Year, month = Month) %>%
  summarise(n = n()) %>%
  filter(n > 1) %>%
  ungroup() 

# Only extract unique observations
chla_ices_unique <- chla_raw_ices %>% 
  distinct(cruise = Cruise, date = Date, Latitude, Longitude, chla_ug_l = Chla_ug_l, 
           depth_m = Bot_Depth, year = Year, month = Month) %>% 
...
# str(secchi_raw_smhi)
# The 'Value' and 'Unit' columns are partly switched, so need to be fixed
fixed_data_smhi <- chla_raw_smhi %>% 
  filter(str_detect(Unit, "Chlorophyll-a bottle")) %>% 
  select(-Parameter)

colnames(fixed_data_smhi) <- setdiff(colnames(chla_raw_smhi), "Quality.flag")
## bind back together with rows that are ok and fix the column classes
new_chla_smhi1 <- bind_rows(
  mutate(fixed_data_smhi, Quality.flag = NA),
  filter(chla_raw_smhi, !str_detect(Unit, "Chlorophyll-a bottle"))
)

####################################
fixed_data_smhi <- new_chla_smhi1 %>% 
  filter(str_detect(Value, "Chlorophyll-a bottle")) %>% 
  select(-Parameter)

colnames(fixed_data_smhi) <- setdiff(colnames(new_chla_smhi1), "Unit")
## bind back together with rows that are ok and fix the column classes
new_chla_smhi2 <- bind_rows(
  mutate(fixed_data_smhi, Unit = NA),
  filter(new_chla_smhi1, !str_detect(Value, "Chlorophyll-a bottle"))
)
####################################
# when making 'Value' as numveric, all the values within that column change
new_chla_smhi <- new_chla_smhi2 %>% 
  mutate(Value = as.numeric(as.character(Value)), Parameter = as.character(Parameter))

# Merge the add basin lookup table to secchi dataset
chla_smhi <- new_chla_smhi %>% 
  rename(chla_ug_l = Value, date = Sampling.date, cruise = Visit.event.identifier,
         # use the decimal degrees for lat and lon (as for ICES)
         Latitude = Sample.latitude..DD., Longitude = Sample.longitude..DD.,
         # 'Station.water.depth' cosidered as 'Bottom depth'?
         Station = Station.name, depth_m = Sample.maximum.depth) %>%
  # is this the right variable for depth?
  mutate(date =  as.Date(date), year = format(date, "%Y"),
         date =  as.Date(date), month = format(date, "%m"),
         #         Month = str_replace(Month, "^0+" ,""),
         year = as.character(year),
         ...
         cruise = as.character(cruise)) %>% 
  #         Station = as.character(Station)) %>%  
  select(cruise, date, Latitude, Longitude, chla_ug_l, depth_m, year, month) 
# %>% 
#  left_join(basin_add_lookup_table, by = c("Latitude", "Longitude")) # 10.649.226

#### Look for duplicate data 
# Check for duplicates in ices
smhi_duplicates <- chla_smhi %>%
  group_by(cruise, date, Latitude, Longitude, chla_ug_l, depth_m, year, month) %>%
  summarise(n = n()) %>%
  filter(n > 1) %>%
  ungroup() 
...
## Use setdiff() to indentify data smhi not in ices  
new_chla_smhi_unique <- setdiff(dplyr::select(chla_smhi_unique, -supplier), dplyr::select(chla_nest_unique, -supplier)) %>%
  mutate(supplier = "smhi") 

## Create a new secchi dataset by only binding the new_smhi_unique object to ices_unique
chla <- bind_rows(chla_smhi_unique, chla_nest_unique, chla_ices_unique) %>% 
  rename(lat = Latitude, lon = Longitude) %>% 
  distinct() # 259480 tot observations 
...
```{r eut nutrient data}
nutrients <- nutrients_rawdata %>% 
  rename(
    no3 = NO3N, no2 = NO2N, nh4 = NH4N, po4 = PO4P, 
    date = OBSDATE, 
    lat = LATITUDE, lon = LONGITUDE, 
    cruise = SHIP, depth = OBSDEP
  ) %>%
