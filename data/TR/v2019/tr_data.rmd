...
basemap + 
  ...
  geom_sf_text(data = nuts2_2016_shp, mapping = aes(label = nuts2), size = 20) %>% 
...
eco_data_combined <- rbind(den_gva, est_gva, fin_gva, ger_gva, lat_gva, lit_gva, pol_gva, swe_gva) %>% 
  ...
  group_by(country) %>% 
  summarize(gva2009 = sum(gva2009, na.rm = TRUE), gva2018 = sum(gva2018, na.rm = TRUE)) %>% 
  ungroup() %>% 
  ...
  select(country, year, coastal_tour_country_gva_estimate) %>% 
  mutate(year = as.character(year))
 

## average accommodatioins per area, area-weighted mean over BHI regions
bhinuts_tourism_avg_coastal <- bhinuts_tourism_coastal %>% 
  st_drop_geometry() %>% 
  ...
  select(region_id, year, avg_accomm_per_area, country = eez) %>% 
  left_join(eco_data_combined, by = c("country", "year")) %>% 
  mutate(accomm_vs_tourism_gva = avg_accomm_per_area/coastal_tour_country_gva_estimate)
```
