...
raw_obs_checklist <- read.csv(file.path(dir_rawdata, "species_obs_w_rgns.csv", stringsAsFactors = FALSE))
...
ggplot(redlist_with_speciesgroup, aes(x=species_group))+
   geom_bar(stat="count")+
   theme(axis.text.x = element_text(colour="grey20",size=8,angle=90,hjust=.5,vjust=.5,face="plain"),axis.text.y = element_text(colour="grey20",size=6))+
  ggtitle("Number of species in the redlist")

# Create list of species with duplicate assessments in redlist
...
full_species <- full_join(species_checklist_no_insect, redlist, by = c("species_id","scientific_name", "species_group"))

dim(full_species) # 3863 rows

```

##### 2.5.3.2 Checking checklist species that are not on redlist

```{r species checklist not redlist}

# Species list on checklist and not redlist

full_species %>% select(-Subbasin) %>% distinct() %>% filter(is.na(list_type.y)) # 68 species
full_species %>% filter(is.na(list_type.y)) %>% select(species_group) %>% distinct() # Inverts, Macrophytes, Birds and Mammals
...
```{r species redlist not checklist}

# Species list on redlist and not checklist

full_species %>% select(-Subbasin) %>% distinct() %>% filter(is.na(list_type.x)) %>% nrow() # 2663 species
full_species %>% filter(is.na(list_type.x)) %>% select(species_group) %>% distinct() # Inverts, Fish, Macrophytes, Birds and Mammals

```

##### 2.5.3.4 Shared Species List

Duplicate assessments were found for 5 species in the shared species list. Upon inspection, duplicate assessments were found to be based off on wintering and breeding statuses. To resolve this, decision was made to take the higher threat assessment for each species.
...
shared_species %>% select(scientific_name) %>% distinct() # 295 species

# Check if there are duplicate assessments (species with more than one threat level assessment)

intersect(unique(shared_species$scientific_name), unique(duplicate_redlist$scientific_name)) # Aythya fuligula, Aythya marila, Melanitta fusca, Mergus serrator, Somateria mollissima

shared_species %>% select(-Subbasin) %>% distinct() %>% filter(scientific_name == "Aythya fuligula" | scientific_name== "Aythya marila" | scientific_name=="Melanitta fusca" | scientific_name=="Mergus serrator" | scientific_name == "Somateria mollissima") # Shows that the duplicate assessment are on 5 bird species that are based off wintering and breeding categories

# Substitute higher threat assessment for each species with duplicate assessments
...
shared_species <- shared_species %>%
                 mutate(red_list_category = ifelse(scientific_name == "Aythya fuligula", "NT - Near Threatened",
                                            ...
                                            ifelse(scientific_name == "Melanitta fusca" , "EN - Endangered",
                                            ifelse(scientific_name == "Mergus serrator", "VU - Vulnerable",
                                            ifelse(scientific_name == "Somateria mollissima", "EN - Endangered", red_list_category)))))) %>%
                 select(-remarks) %>%
                 distinct()

# Recheck for duplicates

shared_species %>% select(scientific_name, red_list_category) %>% distinct() %>% nrow() # 295, same as no. of species
