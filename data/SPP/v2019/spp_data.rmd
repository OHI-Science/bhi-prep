...
raw_species_checklist <- sf::st_drop_geometry(spp_w_rgns)
...
ggplot(raw_species_checklist, aes(x=species_group))+
   geom_bar(stat="count")+
...
raw_species_checklist %>% select(scientific_name)%>%distinct()%>%nrow() #2551 (Including Bacteria and Plankton)
...
basin_ID_namelist <- raw_species_checklist %>%
...
basins_distinct <- raw_species_checklist %>% 
  select(Subbasin) %>% 
  distinct()
...
raw_species_checklist <- raw_species_checklist %>% 
  mutate(Subbasin = as.character(Subbasin)) %>%
  mutate(Subbasin = ifelse(Subbasin == "Bothian Sea", "Bothnian Sea", Subbasin))

raw_species_checklist %>% select(Subbasin) %>% distinct() %>% nrow()  #17 = Correct number
...
species_info <- raw_species_checklist %>% 
  select(
    species_id, 
    scientific_name,
    taxonomica, 
