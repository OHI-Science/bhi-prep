...
**Species Presence, Observations and Red List Status**  
<!-- dataset save location BHI_share/BHI 2.0/Goals/BD/Goals/baltic_species/baltic_species.shp -->
...
dir_B <- "C:/Users/chest/Downloads"  # Edit 1
...
dir_B <- "C:/Users/chest/OneDrive/Documents/Thesis/Github/Shapefiles" # Edit 1
...
### 2.3 Organizing Data to Match 2015 Format

#### 2.3.1 Reorganise into Presence/Absence
...
#### 2.3.2 Isolating and matching BHI ID and basins in raw data

```{r create list BHI ID and Basins}

basin_ID_namelist <- spp_w_rgns %>%
  sf::st_drop_geometry() %>%
...
spp_w_rgns %>% sf::st_drop_geometry() %>% select(Subbasin) %>% distinct() %>% nrow()  #18 basins -> One more than 2015 code

# To check discrepancy

basins <- spp_w_rgns %>% sf::st_drop_geometry() %>% select(Subbasin) %>% arrange(Subbasin = as.character(Subbasin)) %>% distinct()   # Discrepancy = spelling error in Bothnian Sea/Bothian Sea
