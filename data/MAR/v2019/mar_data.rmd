...
mar_datalayer <- combined_gf %>%
  ...
  select(region_id, year, produced_tonnes, )
mar_gapfilling <- combined_gf %>%


write_csv(mar_datalayer)
write_csv(mar_gapfilling)
...
```{r}
## Check relationships between production and nutrients VS estimated values

## check relationships between production and nutrients
## quite linear, so probably ok to gapfill using this relationship...
estim_p_mdl <- lm(estim_p_kg ~ produced_tonnes, data = combined_gf) # summary(estim_p_mdl)
estim_n_mdl <- lm(estim_n_kg ~ produced_tonnes, data = combined_gf) # summary(estim_n_mdl)

plotly::ggplotly(
  ggplot(combined_gf) + 
    geom_abline(
      slope = coef(estim_p_mdl)[["produced_tonnes"]],
      intercept = coef(estim_p_mdl)[["(Intercept)"]], 
      color = "grey"
    ) +
    geom_point(aes(x = produced_tonnes, y = estim_p_kg), color = "dark green", alpha = 0.75)
)

## Check distribution of actual and estimated values
# actual values
ggplot(combined_gf, aes(x=p_kg)) + 
  geom_histogram(color="darkblue", fill="lightblue")

#estimated values
ggplot(combined_gf, aes(x=estim_p_kg)) + 
  geom_histogram(color="darkblue", fill="lightblue")
