...
### NOTE: There will be an issue related to the bhi_regions no and the farm_ID 
### for big areas, such as "Other_coasts" for Sweden, where multiple bhi_regions are considered 
...
combined_gf <- combined_rawdata %>% 
  ...
  mutate(
    gapfill_nutrients = ifelse(is.na(p_kg) & is.na(n_kg), TRUE, FALSE),
    gapfill_prod = ifelse(is.na(produced_tonnes), TRUE, FALSE),
    small_farm = ifelse(produced_tonnes <= 20 | n_kg < 100, TRUE, FALSE)
  ) %>% 
