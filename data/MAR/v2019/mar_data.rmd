...
bhiregions <- bind_rows(
  ...
  c(country = "DK", lvl = "subnatl", area = "Hovedstaden", rgn = list(2, 12, 15)),
  c(country = "DK", lvl = "subnatl", area = "Sjælland", rgn = list(2, 3, 7, 9, 12)),
...
combined_rawdata <- combined_rawdata %>%
  ...
  ungroup() %>% 
  mutate(
    rgn_production = ifelse(level == 2, produced_tonnes*area_frac, NA),
    rgn_N = ifelse(level == 2, n_kg*area_frac, NA),
    rgn_P = ifelse(level == 2, p_kg*area_frac, NA)
  ) %>%
  group_by(year, country, farm_species) %>% 
  mutate(
    subnatl_prod = sum(ifelse(source == "national", rgn_production, 0), na.rm = TRUE),
    subnatl_N = sum(ifelse(source == "national", rgn_N, 0), na.rm = TRUE),
    subnatl_P = sum(ifelse(source == "national", rgn_P, 0), na.rm = TRUE)
  ) %>% 
  ungroup() %>% 
  mutate(
    rgn_production = ifelse(level == 1, (produced_tonnes-subnatl_prod)*area_frac, rgn_production),
    rgn_N = ifelse(level == 1, (n_kg-subnatl_N)*area_frac, rgn_N),
    rgn_P = ifelse(level == 1, (p_kg-subnatl_P)*area_frac, rgn_P)
  ) %>% 
  select(
    ...
    rgn_production, rgn_production, rgn_N, rgn_P
  )
