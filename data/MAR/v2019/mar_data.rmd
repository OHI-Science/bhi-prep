...
plotdf <- rbind(rbt_raw, finfish_gather_yr) %>% 
  ...
  ## to use predict function later based on lm obj, need rgn names in models
  dplyr::rename(rgn_production = produced_tonnes, rgn_N = n_kg, rgn_P = p_kg)

## check relationships between production and nutrients
## quite linear, so probably ok to gapfill using this relationship...
p_mdl <- lm(rgn_P ~ 0 + rgn_production, data = plotdf) # summary(p_mdl)
n_mdl <- lm(rgn_N ~ 0 + rgn_production, data = plotdf) # summary(n_mdl)

plotly::ggplotly(
  ggplot(plotdf) +
    geom_abline(slope = coef(p_mdl)[["rgn_production"]], intercept = 0, color = "grey") +
    geom_point(aes(x = rgn_production, y = rgn_P), color = "blue", alpha = 0.75)
)

## production estimates with linear models
## strange things happen when including phosphorus (negative coefficient...)
prod_mdl <- lm(rgn_production ~ 0 + rgn_N, data = plotdf) # summary(prod_mdl)

plotly::ggplotly(
  ggplot(plotdf) +
    geom_abline(slope = coef(prod_mdl)[["rgn_N"]], intercept = 0, color = "grey") +
    geom_point(aes(x = rgn_N, y = rgn_production), color = "blue", alpha = 0.75)
)
```
...
mar_gapfilling <- combined_rawdata %>% 
  ...
  mutate(
    gapfill_nutrients = (is.na(rgn_P)|is.na(rgn_N)) & !is.na(rgn_production),
    gapfill_prod = is.na(rgn_production) & !is.na(rgn_N) & !is.na(rgn_P)
  ) %>% 
  ...
  mutate(
    rgn_N = ifelse(is.na(rgn_N), estim_n_kg, rgn_N),
    rgn_P = ifelse(is.na(rgn_P), estim_p_kg, rgn_P),
    rgn_production = ifelse(gapfill_prod, estim_prod, rgn_production)
  ) %>%
  ...
  mutate(
    ...
    N_prod_ratio = ifelse(rgn_production == 0, 0, rgn_N/rgn_production),
    P_prod_ratio = ifelse(rgn_production == 0, 0, rgn_P/rgn_production)
  ) %>% 
  mutate(
  ...
  select(
    year, region_id, farm_species, 
    gapfilled, gapfill_nutrients, gapfill_prod, 
    estim_n_kg, estim_p_kg, estim_prod, 
    rgn_N, rgn_P, rgn_production,
...
mar_datalayers <- mar_gapfilling %>% 
  ...
  summarize(
    n_kg = sum(rgn_N, na.rm = TRUE),
    p_kg = sum(rgn_P, na.rm = TRUE),
    produced_tonnes = sum(rgn_production, na.rm = TRUE),
    n_ratio = mean(N_prod_ratio, na.rm = TRUE),
    p_ratio = mean(P_prod_ratio, na.rm = TRUE),
    percent_gf = sum(gapfilled)/n()
  )
