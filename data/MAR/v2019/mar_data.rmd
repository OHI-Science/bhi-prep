...
combined_gf <- combined_rawdata %>% 
  ...
  mutate(
    ## gapfill nitrogen
    gfN = ifelse(
      !gapfill_nutrients, n_kg,
      ifelse(small_farm, estim_n_small, estim_n_kg)
    ),
    ## gapfill phosphorus
    gfP = ifelse(
      !gapfill_nutrients, p_kg,
      ifelse(small_farm, estim_p_small, estim_p_kg)
    )
  )
# chkdata <- combined_gf %>% 
#   select(produced_tonnes,n_kg, gfN) %>% 
#   tidyr::pivot_longer(cols = c(n_kg, gfN))
# plotly::ggplotly(ggplot(chkdata) + geom_point(aes(produced_tonnes, value, color = name)))
