...
areasMap <- ggplot2::ggplot() + 
  ...
  scale_fill_manual(
    values = colorRampPalette(cols)(51)
  ) +
...
```{r merge two finish datasets, echo = TRUE}
## two datasets:
...
ggplot(mar_finland_totals, aes(produced_tonnes)) +
  geom_histogram(binwidth = 400, fill = "blue", alpha = 0.5, color = "grey") +
  facet_wrap( ~ area, nrow = 2)

boxplot(produced_tonnes ~ area, mar_finland_totals)
...
## rainbow trout finland, add back years where don't have area specific data
...
```{r investigate nutrient estimates with linear models, results = "show", message = FALSE, echo = TRUE, fig.width = 9.5}
...
```{r explore raw data by source and country, results = "show", message = FALSE, echo = TRUE, fig.width = 9.5}
datasources <- ggplot(
  combined_rawdata %>% 
...
```{r histograms of nutrients allowances minus nutrients values, results = "show", message = FALSE, echo = TRUE, fig.width = 9.5}
...
## positive values are in excess of the allowance
## helcom recommnedation is 50gN and 7kgP per kg of fish
hist_all <- ggplot(
  plotdf %>% 
    mutate(diffsN = Nmax - Nitrogen, diffsP = Pmax - Phosphorus) %>% 
    tidyr::pivot_longer(cols = c(Nitrogen, Phosphorus, diffsN, diffsP, Production)) %>% 
    filter(name %in% c("diffsN", "diffsP"))
  ) +
  geom_histogram(aes(value), color = "blue", fill = NA, binwidth = 500) +
  facet_wrap(~ name, nrow = 2)

hist_lessthan0 <- ggplot(
  plotdf %>% 
    mutate(diffsN = Nmax - Nitrogen, diffsP = Pmax - Phosphorus) %>% 
    filter(diffsN < 0, diffsP < 0) %>%
    tidyr::pivot_longer(cols = c(Nitrogen, Phosphorus, diffsN, diffsP, Production)) %>% 
    filter(name %in% c("diffsN", "diffsP"))
  ) +
...
gridExtra::grid.arrange(hist_all, hist_lessthan0, nrow = 1)
```
...
```{r gapfilled percentages by country and region id, results = "show", message = FALSE, echo = TRUE, fig.width = 9.5}
percentagegf <- ggplot(
  mar_datalayers %>% 
    left_join(
      bhiregions %>% 
...
leaflet(data = plotshp) %>% 
  ...
  ## data gapfill percentages
  # addPolygons(
  #   stroke = TRUE, opacity = 0.3, weight = 1, fillOpacity = 0.7,
  #   fillColor = ~pal_gf(pct_prod_gf),
  #   group = "Production data gapfilled (proportion)"
  # ) %>% 
  # addPolygons(
  #   stroke = TRUE, opacity = 0.3, weight = 1, fillOpacity = 0.7,
  #   fillColor = ~pal_gf(pct_nutrient_gf),
  #   group = "Nutrient data gapfilled (proportion)"
  # ) %>% 
  ...
  addPolygons(
    popup = paste(
      ...
      round(plotshp$produced_tonnes, 3), "</h5>",
