...
combined_rawdata <- rbind(rbt_raw2, finfish_raw3) %>% 
...
combined_raw <- combined_rawdata  %>% 
  mutate(value_p_kg = p_kg/(produced_tonnes*1000),
         value_n_kg = n_kg/(produced_tonnes*1000)
         ) 
## create average values by area
combined_raw1 <- combined_raw %>% 
  group_by(area) %>%
