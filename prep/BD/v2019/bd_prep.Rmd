...
for(shp in c("wintering_birds_shp", "breeding_birds_shp")){
  ...
  birdshp <- birdshp %>% 
    ...
    mutate(wading_feeders = as.numeric(wading_feeders), surface_feeders = as.numeric(surface_feeders))
...
for(shp in c("benthic_shp", "pelagic_shp")){# "fish_shp", "seals_shp")){
  ...
  if(shp == "fish_shp"){
    ...
    #   st_buffer(filter(get(shp), country == "Opensea"), 0), 
    #   st_transform(BHI_rgns_shp, crs = 3035)
    # )
    opensea_w_rgns <- sf::st_intersection(
      st_make_valid(filter(get(shp), country == "Opensea")), 
      st_transform(BHI_rgns_shp %>% rename(subbasin = Subbasin), crs = 3035)
    )
    opensea_w_rgns <- opensea_w_rgns %>% 
      select(region_id = BHI_ID, region_name = rgn_nam, subbasin, country, region_area_km2 = Area_km2) %>% 
