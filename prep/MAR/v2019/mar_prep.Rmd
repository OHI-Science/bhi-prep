...
- `mar_harvest_bhi2019.csv` 
- `mar_status_bhi2019.csv` 
- `mar_trend_bhi2019.csv` 
...
mar_datalayers <- mar_gapfilling %>% 
  ...
  summarize(
    n_kg = round(sum(split_N_gf, na.rm = TRUE), 3),
    p_kg = round(sum(split_P_gf, na.rm = TRUE), 3),
    produced_tonnes = round(sum(split_prod_gf, na.rm = TRUE), 3),
...
As noted above, the HELCOM recommnedation is 50gN and 7gP per kg of fish. The plots below explore differences between recorded and recommended nutrient use. The histogram plots show the distributions of differences with max allowable nutrients minus actual or estimated, so negative values are in excess of the recommendation.
...
plotdf <- mar_datalayers %>% 
  ...
  mutate(
    region_id = as.factor(region_id), #somehow, without doing the conversion, the values look more normal (?)
    diffsN = 50*produced_tonnes - n_kg, # if converting: ref. value = 50g --> 0.05kg
    diffsP = 7*produced_tonnes - p_kg,  # ref. value = 7g --> 0.007kg
...
- In goal calculation, data are taken as 5 year running mean.
- Data unit kg of nutrients/tonnes of fish produced.
- Reference point is the [nutrient limit values](http://www.helcom.fi/Recommendations/Rec%2025-4.pdf) per fish produced.

```{r mar status}
#calculate ref nutrient values used to fish produced
    
mar_calc <- mar_datalayers %>% 
  select(-pct_prod_gf, -pct_nutrient_gf, -pct_gf) %>%
  # if converting to kg, the values look weird (?)
  mutate(ref_n_kg = 50*produced_tonnes,
         ref_p_kg = 7*produced_tonnes)

## Calculate status
mar_status_score <- mar_calc %>% 
  filter(year >= 2014) %>% 
  group_by(rgn_id = region_id, year) %>%
  mutate(n_status = round(pmin(1, ref_n_kg/n_kg) * 100, 2), #calculate status per year
         p_status = round(pmin(1, ref_p_kg/p_kg) * 100, 2),
         score = mean(c(n_status, p_status), na.rm = T),
         dimension = 'status') %>% 
  ungroup() %>% 
  dplyr::select(rgn_id, year, score, dimension) 

# save the status layer
write_csv(
  mar_status_score,
  file.path(dir_layers, "mar_status_bhi2019.csv"))
```
...
mar_trend <- mar_status_score %>%
  ...
  do({if(sum(!is.na(.$score)) >= 5)      #calculate trend only if X years of data (min_regr_length) in the last Y years of time serie (regr_length)
    data.frame(score = max(-1, min(1, coef(lm(score ~ year, .))['year'] * 0.05))) #future_year set in contants, this is the value 5 in the old code
    else data.frame(score = NA)}) %>%
  ungroup() %>%
  mutate(score = round(score, 2),
         dimension = 'trend') 

# save trend layer
write_csv(mar_trend, file.path(dir_layers, 'mar_trend_bhi2019.csv')) 
```
