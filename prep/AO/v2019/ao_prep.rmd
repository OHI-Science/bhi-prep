...
# Use the basin lookup table for now, as the above function is not working 
coastalfish_rgns <- combined_coastal_fish1 %>% 
...
bhi_status <- bhi_mean_score %>%
  dplyr::select(bhi_id, mean_basin_score, threshold_value) %>%
  mutate(
...
# library(broom)
cpue_mdl <- cpue_trans %>%
  dplyr::select(Subbasin, Country, monitoring_area, core_indicator, Taxa, Year, cpue_z) %>%
...
slope2 <- slope %>%
  dplyr::select(Subbasin, Country, core_indicator, Taxa, slope_mean_basin_taxa)
```
...
slope3 <- slope2 %>%
  dplyr::select(Subbasin, core_indicator, slope_mean_basin_indicator)
```
...
slope4 <- slope3 %>%
  dplyr::select(Subbasin, slope_mean_basin) %>%
  mutate(
...
bhi_slope <- slope4 %>%
  dplyr::select(bhi_id, slope_mean_basin)%>%
  dplyr::rename(rgn_id = bhi_id,
