...
```{r mean slope by group per bhi region, eval = FALSE, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 5.5}
...
ggplot(slope_bhi) + 
  geom_hline(yintercept = 0) +
  geom_boxplot(aes(bhi_id, slope)) +
  geom_point(aes(bhi_id, slope_mean_bhi_taxa), colour = "red") +
  facet_wrap(~core_indicator + Taxa)+
  theme(axis.text.x = element_text(colour = "grey20", size = 8, angle = 90,
                                   hjust = .5, vjust = .5, face = "plain")) +
...
slope <- slope_bhi %>%
  dplyr::select(bhi_id, core_indicator, Taxa, slope_mean_bhi_taxa)
```
...
ggplot(slope2) + 
  geom_hline(yintercept = 0) +
  geom_point(aes(bhi_id, slope_mean_bhi_indicator), colour = "red") +
  facet_wrap(~core_indicator) +
  theme(axis.text.x = element_text(colour = "grey20", size = 8, angle = 90,
                                   hjust = .5, vjust = .5, face = "plain")) +
  ylim(-.3,.3) +
...
slope3 <- slope2 %>%
  dplyr::select(bhi_id, core_indicator, slope_mean_bhi_indicator)
```
...
ggplot(slope3) + 
  geom_hline(yintercept = 0) +
  geom_point(aes(bhi_id, slope_mean_bhi), colour = "black", size = 3) +
  geom_point(aes(bhi_id, slope_mean_bhi_indicator, shape = core_indicator), colour = "red") +
  theme(axis.text.x = element_text(colour = "grey20", size = 8, angle = 90,
                                   hjust = .5, vjust = .5, face = "plain")) +
...
### 5.6 Merge basin lookup table to BHI Region
