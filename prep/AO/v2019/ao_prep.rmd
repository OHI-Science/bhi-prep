...
monitoring_indicator_mean <- combined_coastal_fish %>% 
  select(
    Subbasin, Monitoring.area, lat, lon, core_indicator, Status.value.monitoring.location, threshold_value
    ) %>%
  group_by(Subbasin, Monitoring.area, lat, lon, core_indicator, threshold_value)%>%
...
monitoring_mean_score <- monitoring_indicator_mean %>%
  group_by(Monitoring.area, Subbasin, lat, lon, threshold_value) %>%
...
```{r plot scores levels of aggregation, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
...
```{r monitoring mean score map, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
...
## Set up the plot
plot_map <- ggmap(map) +
  geom_point(aes(x = lon, y = lat, colour = mean_core_monitoring_score, 
                 shape = factor(core_indicator)), data = map_data, size = 2.5)
...
```{r plot basin indicator scores, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
...
```{r plot basin mean across indicators, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
## Plot basin mean score across indicators
...
There are no scores for Kiel Bay and Gdansk Basin, therefore no scores for BHI regions 7, 8, 18, 19. 
For Kiel Bay, this could be because some monitoring locations should be assigned to Kiel Bay but are not. There are no Polish data which is why there is no scoring for Gdansk Basin.  
...
- Although scores is calculated by basin, Jens Olsson suggests not applying the score to regions where no sampling occurred. Therefore, 13 regions have the score replaced with NA (7, 8, 18, 19)
...
```{r  final score object plotted, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
...
ggplot(plot_score) +
  ...
  geom_text(aes(rgn_nam, score, label = score), color = "slategray", size = 3) +
  coord_flip() +
  labs(x = NULL, y = "BHI regions") +
  theme_bw() +
...
We decided to follow the same approach as for BHI 1.0, from [Bergström et al 2016](https://www.researchgate.net/publication/288058474_Long_term_changes_in_the_status_of_coastal_fish_in_the_Baltic_Sea), but only focus on the final time period
for the slope (2004-2016)

### 5.1 Trend calculation method {-}

STEPS:
1.Select final time period of trend assessment (2004-2016)
...
```{r select time period and plot data, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
## Select years 2004-2016
cpue <- cpue %>%
...
ggplot(filter(cpue, core_indicator == "Key species")) +
  ...
  ggtitle("Key Species CPUE (2006-2016)")

ggplot(filter(cpue, Taxa == "Cyprinids")) +
  geom_point(aes(Year, cpue)) +
  facet_wrap(~monitoring_area, scales="free_y")+
  ggtitle("Functional Group Cyprinids CPUE (2006-2016)")

ggplot(filter(cpue, Taxa == "Piscivores")) +
  geom_point(aes(Year, cpue)) +
  facet_wrap(~monitoring_area, scales="free_y")+
  ggtitle("Functional Group Piscivores CPUE (2006-2016)")
```
<br/>

**Transform data**
```{r transform data, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
...
**Plot slope**
```{r plot slope, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
...
ggplot(cpue_mdl) +
  ...
  geom_hline(yintercept = 0) +
  geom_text(aes(Subbasin, slope, label = nobs), size = 2) +
  theme(axis.text.x = element_text(colour = "grey20", size = 8, angle = 90,
                                   hjust = .5, vjust = .5, face = "plain")) +
  guides(colour = "none") +
  ggtitle("lm slopes 2006-2016")

## Plot without number of observations
ggplot(cpue_mdl) +
  geom_point(aes(Subbasin, slope, colour = monitoring_area,
                 shape = core_indicator), size = 2) +
  geom_hline(yintercept = 0) +
  theme(axis.text.x = element_text(colour = "grey20", size = 8, angle = 90,
                                   hjust = .5, vjust = .5, face = "plain")) +
  guides(colour = "none") +
...
```{r plot slopes, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
...
```{r mean slope by group per basin, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
## slope
slope <- cpue_mdl %>%
...
slope2 <- slope %>%
  select(Subbasin, core_indicator, Taxa, slope_mean_basin_taxa)
```
...
```{r  mean indicator slope, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
slope2 <- slope2 %>%
...
```{r basin mean slope, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
slope3 <- slope3 %>%
  group_by(Subbasin) %>%
...
```{r plot final code object, eval = FALSE, echo = TRUE, message = FALSE, warning = FALSE}
...
## Status Score
write.csv(bhi_score, here::here("layers", 'ao_stock_status_bhi2019.csv'), row.names=FALSE)
