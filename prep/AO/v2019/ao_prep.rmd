...
# library(stringi)
combined_coastal_fish <- combined_coastal_fish %>% 
  rename(lat = Latitude, lon = Longitude) %>% 
  filter(!is.na(lat) & !is.na(lon)) %>%  # however, by doing this, more than 70 locations will be cut off
  mutate(
...
coastalfish_rgns <- join_rgns_info(
  combined_coastal_fish, helcomID_col = "helcom_id", country_col = "country", 
...
basin_n_obs <- combined_coastal_fish %>% 
  select(Subbasin) %>%
  count(Subbasin)

## Indicator mean by monitoring area
monitoring_indicator_mean <- combined_coastal_fish %>% 
  select(
