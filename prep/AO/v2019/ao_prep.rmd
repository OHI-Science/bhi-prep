...
dir_ao <- file.path(here::here(), "prep/AO/v2019")
assess_year <- v2019
...
## 5.0 Trend Calculation for AO Stations
...
Do steps 1-3 in this code, export slopes for each monitoring area. Slopes added to level 1 in database, extract slopes from database and finish up steps 4-8 in the ao_prep.rmd file.
...
source(file.path(here::here(), "R/ao_prep_fun.R"))

cpue <- read.csv('C:/Users/jgrif/Documents/StockholmUnivPostDoc/BalticHealthIndex/DataDownloads/AO/ao_coastal_fish_cpue.csv', sep = ";")
...
```{r transform and calculate slopes}
cpue_trans = cpue %>%
...
## Linear model and extracting slope
...
cpue_mdl %>% select(nobs) %>% min()
```


```{r visualization and data checking}
