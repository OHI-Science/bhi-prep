...
```{r data, child = data_path, results = "asis", echo = FALSE, fig.width = 9.5}
```
...
combined_w_rgns <- combined_w_rgns %>% filter(species == "cod") # UNCOMMENT FOR SCORES BASED ON ONLY COD STOCKS...
...
for(g in goal_stocks){
  ...
  if(g == "FIS"){
    for(s in c("spr", "her")){
      fis_data <- fis_data %>% filter(!str_detect(stock, s))
      g <- ifelse(s == "her", "FIS_COD", "FIS")
    }
...
```{r compare to data used in last assessment, fig.height = 3.8}

## load layers
fis_bbmsy <- read_csv(file.path(
  dir_layers, 
  sprintf("v%s", assess_year), 
...
write_csv(
  ...
  file.path(dir_layers, bhi_version, sprintf("fis_cod_penalty_bhi%s.csv", assess_year))
)
```
<br>
...
```{r evaluate goal model and data, fig.width = 10, fig.height = 8}
...
```{r, out.width = "70%", results = "show", echo = FALSE}
include_graphics("../fis-fscores-formula.png")
```
