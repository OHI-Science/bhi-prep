...
ggplot(data = q4_fultonsK) +
  geom_point(aes(x = year, y = avg_cond, color = above_one), size = 2, alpha = 0.6) +
...
## from ohi-global functions.R, Oct 22, 2019
...
B_scores <- metric_scores %>%
  mutate(
    score_type = "B_score",
    score = ifelse(
      bbmsy < lowerBuffer, bbmsy,
      ifelse(
        bbmsy >= lowerBuffer & bbmsy < upperBuffer, 1, 
        ifelse(bbmsy >= upperBuffer, (3.35 - bbmsy)/(3 - upperBuffer), NA)
      )
    )
  ) %>% 
  mutate(
...
score = ifelse(
  ...
  bbmsy,
  ifelse(
    bbmsy >= lowerBuffer & bbmsy <= upperBuffer, 1, NA
  )
)
