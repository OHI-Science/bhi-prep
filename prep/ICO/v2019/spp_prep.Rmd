---
title: "Species - Biodiversity Subgoal data prep"
output:
  html_document:
    toc: true
    toc_depth: 4
...
```{r spp biodiversity preamble prep, message = FALSE}
...
```{r biodiversity background, child = bkgd_path, results = "asis", echo = FALSE}
```
...
```{r biodiversity data, child = data_path, results = "asis", echo = FALSE}
```
...
shared_list %>% 
  ...
  select(region_id, year = assessment_year_red_list, scientific_name, species_group, red_list_category) %>% 
...
shared_list %>% 
  ...
  select(region_id, year = assessment_year_red_list, scientific_name) %>% 
...
spp_status_by_taxa_penalized <- spp_by_taxa %>% 
  ...
  summarize(
    ...
    status_basin_initial = exp(weighted.mean(log(status_taxa_initial), nspp)),
...
basemap + 
  ...
  scale_fill_gradientn(colors = statusmapcols, limits = c(70, 100), na.value = "gainsboro") +
  theme(
