---
title: "Iconic Species - Sense of Place Subgoal data prep"
output:
  html_document:
    toc: true
    toc_depth: 4
...
missing_fis <- data_frame(
  scientific_name = c(
    ...
    "Esox lucius", "Salmo trutta", "Sprattus sprattus"
  ),
  Subbasin = c(
    list(subbasins[1:17]), list(subbasins[1:3]), list(subbasins[1:12]), 
    list(subbasins[1:17]), list(subbasins[1:3]), list(subbasins[1:12]), 
    list(subbasins[1]), 
    list(subbasins[2:6]), 
    list(subbasins[6:12]),
    list(subbasins[13:17]), list(subbasins[1:3]), list(subbasins[1:11])
  ),
  no_rec_this_dec = rep(FALSE, 12),
  ## cod has 2007 and a 2013 assessment
  assessment_year_red_list = c(rep(2007, 3), rep(2013, 9))
)

missing_fis <- tidyr::unnest(missing_fis, cols = c(Subbasin))

# missing_mammals <- data_frame()
...
## all species data with assessment (iucn) categories
sppdata <- rbind(
...
```{r select ico spp}
ico_spp_data <- left_join(ico_spp_list, sppdata,  by = c("latin_name" = "scientific_name"))
...
ico_spp_w_birds %>% 
  ...
  ggplot() +
    geom_bar(aes(latin_name, presence), stat = "identity") +
