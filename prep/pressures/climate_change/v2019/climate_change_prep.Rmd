...
cc_data_w_rgns_sp <- cc_data_w_rgns_sp %>% 
  ...
  # filter((depth <= 40)|(!is.na(surfsal) & is.na(depth))) %>% 
...
surf_temp_current <- cc_data_w_rgns %>%
  filter(month %in% 7:8) %>% 
  group_by(Subbasin, year) %>% 
  summarize(julaug_mean_temp = mean(temp, na.rm = TRUE)) %>% 
  arrange(Subbasin, year) %>% 
  group_by(Subbasin) %>%
  mutate(
...
load_files <- list.files(
  file.path(dir_B, "Pressure", "Projections"), 
...
surf_sal_prs <- dplyr::left_join(d, surf_sal_prs, by = c("region_id", "year"))
surf_temps_prs <- dplyr::left_join(d, surf_temps_prs, by = c("region_id", "year"))

write.csv(surf_sal_prs, file.path(dir_layers, "cc_sal_surf_bhi2019.csv"), row.names = FALSE)
write.csv(surf_temps_prs, file.path(dir_layers, "cc_sst_bhi2019.csv"), row.names = FALSE)
```
