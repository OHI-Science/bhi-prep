...
For the secchi depth indicator, only offshore data points will be used. These are identified using [HELCOM Coastal and Offshore divisions (2018 version)](https://maps.helcom.fi/arcgis/rest/services/MADS/Sea_environmental_monitoring/MapServer/4).
...
plot_secchi_bhi_ts <- ggplot(secchi_offshore) + 
  geom_point(
    aes(year, secchi_m, color = supplier), 
...
knitr::kable(
  secchi_last_year %>% 
    group_by(last_year) %>%
    summarize(num_basins_w_year = n(), basins = paste(Subbasin, collapse = ", "))
)
...
ggplot(secchi_status %>% left_join(secchi_last_year, by = c("subbasin" = "Subbasin"))) +
  ...
  geom_text(aes(region, score, label = last_year)) +
  coord_flip() +
  labs(x = NULL, y = NULL) +
  theme_bw() +
...
## linear model method 1, from the basin mean secchi values ----
...
## linear model method 2, from observation values ----
...
#   left_join(select(eut_thresholds, subbasin = basin, target_secchi = summer_secchi))

## trend plots ----

trendplotdf <- left_join(
  secchi_offshore %>% 
    rename(subbasin = Subbasin) %>% 
    filter(year %in% trendyrs),
  secchi_lm_estim %>%
...
secchi_trend <- secchi_lm_estim %>% 
    mutate(
      ...
      trend_score = max(-1, min(1, expectedchange5yrs))
      ## alternatively, could calculate as relative change w.r.t subbasin threshold:
...
Basin status is initially a value between 0 and 1, later rescaled to the 0 to 100 range. The status is calculated for each (data) year between 2000 and 2019.
...
For the chlorophyll a indicator, coasta and offshore data points will be used. These are distinguished using [HELCOM Coastal and Offshore divisions (2018 version)](https://maps.helcom.fi/arcgis/rest/services/MADS/Sea_environmental_monitoring/MapServer/4).
...
Along with the mean calculation, we calculate five-year moving averages for each year in the dataset. The moving average (or 'rolling mean') is an average of summer means from the current and previous four years. 
...
Status for the _chlorophyll a_ indicator is calculated as `basin target` divided by `basin summer mean`, with the value capped at 1. We calculate status for each Coastal, Offshore, and Combined categories for evaluation, but select the Combined version for the _chlorophyll a_ indicator status. The plot shows BHI region scores (uniform across regions within the same basin, as the data is aggregated by basin for the summer means calculation), and also indicates which data year the status is based on.
...
The trend plots show chlorophyll a measurements by basin, with a red line indicating where the linear model predicts mean secchi depths will be five years in the future, and a blue line indicating current assessment year summer mean. Vertical lines frame the five-year time frame over which trend is projected.

```{r eut chl a trend calculation, results = "show", warning = FALSE, fig.width = 9.5, fig.height = 14}
trendyrs <- min_year:(min_year + regr_length)

## linear model method 1, from the basin mean secchi values ----

chla_lm_estim <- chla_status_allyrs %>%
  distinct(subbasin, year, coastal, mean_chla) %>%
...
#   left_join(select(eut_thresholds, subbasin = basin, target_chla = summer_chla))
...
ggplot(trendplotdf) +
  ...
  # facet_wrap(
  #   c("subbasin", "coastal"), 
  #   labeller = label_wrap_gen(width = 70, multi_line = FALSE), 
  #   scales = "free_y", 
  #   ncol = 3
  # ) +
  facet_grid(rows = vars(subbasin), cols = vars(coastal)) +
  labs(x = NULL, y = NULL) +
  ggtitle("Chlorophyll a with 5 Years in Future and Current Estimates (Red and Blue lines Respectively)")
...
```{r chlorphyl a status and trend for regions plot, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 4}
...
for(r in c("Subbasin", "BHI_ID")){
  ...
  plotboxplot <- ggplot(mutate(plotdf, month = as.factor(month))) +
    ...
    labs(x = "\nMonth", y = NULL, fill = "Coastal/Offshore") +
    ggtitle("Monthly DIN and DIP") +
...
lapply(
  ...
  function(x){
    ...
    p <- ggplot(plotdf) +
      ...
      ggtitle(paste(
        x[[1]], x[[2]], 
        "with 5 Years in Future and Current Estimates (Red and Grey lines Respectively)"
      ))
...
gridExtra::grid.arrange(
  offshore_lm_plot_din, coastal_lm_plot_din,
  offshore_lm_plot_dip, coastal_lm_plot_dip,
  nrow = 4
)
...
```{r eut nutrients status and trend maps, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 4}
