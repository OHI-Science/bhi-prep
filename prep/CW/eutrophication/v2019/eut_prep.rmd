...
## import offshore helcom codes,  used for Secchi and Chlorophyl a
...
```{r secchi timeseries yearly and monthly, results = "show", echo = TRUE, fig.width = 11, fig.height = 5}
## Plot by BHI regions and year
plot_secchi_bhi_ts <- ggplot(secchi_offshore) + 
  geom_point(aes(year, secchi_m, color = supplier), size = 1, alpha = 0.3) +
  facet_wrap(
      c("Subbasin", "rgn_nam"), 
...
plot_secchi_bhi_box <- ggplot(mutate(secchi_offshore, month = as.factor(month))) + 
  geom_boxplot(aes(month, secchi_m, fill = supplier), size = 0.2, outlier.size = 0.3, alpha = 0.6) +
  facet_wrap(
...
secchimeansplotsubbasin + ggtitle("Mean Summer Secchi (June-Sept, Offshore) against Target Values")
secchimeansplotbhi_id + ggtitle("Mean Summer Secchi (June-Sept, Offshore) against Target Values")
```
...
```{r chla offshore coastal data maps, results = "show", echo = TRUE, fig.width = 15, fig.height = 5}
...
```{r chl a timeseries yearly and monthly, results = "show", echo = TRUE, fig.width = 12.5, fig.height = 7}
plotinfo <- list(
  ## by BHI regions
  list("chla_coastal", c("BHI_ID", "Subbasin", "rgn_nam"), "Chlorophyl a (Coastal) by BHI Region", "bhi"),
  list("chla_offshore", c("BHI_ID", "Subbasin", "rgn_nam"), "Chlorophyl a (Offshore) by BHI Region", "bhi"),
  list("chla_whole", c("BHI_ID", "Subbasin", "rgn_nam"), "Chlorophyl a by BHI Region", "bhi"),
  ## by Subbasins
  list("chla_coastal", "Subbasin", "Chlorophyl a (Coastal) by Subbasin", "basin"),
  list("chla_offshore", "Subbasin", "Chlorophyl a (Offshore) by Subbasin", "basin"),
  list("chla_whole", "Subbasin", "Chlorophyl a by Subbasin", "basin")
)
...
lapply(
  ...
  function(x){
    ...
    plottimeseries <- ggplot(full_join(get(x[[1]]), expandcases)) +
      geom_point(aes(year, chla_ug_l, color = supplier), size = 0.5, alpha = 0.5) +
...
lapply(
  ...
  function(x){
    ## 1. evaluate data by source
    plotboxplot <- ggplot(get(x[[1]]) %>% full_join(expandcases) %>% mutate(month = as.factor(month))) +
      geom_boxplot(aes(month, chla_ug_l, fill = supplier), size = 0.2, outlier.size = 0.3, alpha = 0.4) +
      labs(x = "Month", y = "Chl a (ug/L)", fill = "Data Source", title = x[[3]]) +
...
gridExtra::grid.arrange(plot_chla_offshore_basin_ts, plot_chla_coastal_basin_ts, nrow = 1)
gridExtra::grid.arrange(plot_chla_offshore_basin_box, plot_chla_coastal_basin_box, nrow = 1)
# gridExtra::grid.arrange(plot_chla_offshore_bhi_box, plot_chla_coastal_bhi_ts, nrow = 1)
# gridExtra::grid.arrange(plot_chla_offshore_bhi_box, plot_chla_coastal_bhi_box, nrow = 1)
```
...
for(r in c("Subbasin", "BHI_ID")){
  ...
  ## join with basin-specific eutrophication chlorophyl a targets
  ...
  chla_meansplot <- ggplot(chla_summer_months_mean, aes(year, mean_chla, group = 1)) + 
    ...
    facet_grid(rows = vars(!!!syms(r)), cols = vars(coastal)) +
    scale_y_continuous(limits = c(0, 10)) +
    labs(x = NULL, y = "Chlorophyl a (ug/L)") +
    ggtitle(sprintf("Mean Summer (June-Sept) Chlorophyl a by %s", r))
...
**Plot summer chlorophyl a with target values indicated**

Horizontal lines are HELCOM target values 

```{r summer chl a with target, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 14}
chla_meansplot_subbasin + ggtitle("Mean Summer Chlorophyl a (June-Sept) against Target values")
```
...
Only five (5) basins have data for 2019, which will be used as the status year, while six (6) have data for 2018 and two (4) for 2016. For these basins, their most recent year's data is used for current status calculations
...
```{r eut chla status calculation, results = "show", echo = TRUE, fig.width = 9.5}
...
ggplot(chla_status) +
  ...
  theme(axis.text.y = element_text(size = 10)) +
  scale_fill_manual(values = statuspal) +
  ggtitle("Chlorophyl a Status by Region, for Coastal, Offshore, and Combined data")

chla_status <- chla_status %>% filter(coastal == "Both Combined") %>% select(-coastal)
```

#### 3.2.4 Trend Calculations: Coastal & Offshore {-}

```{r eut chl a trend calculation, results = "hide", fig.width = 9.5}
## basin trend regression ----
chla_trend <- chla_status_allyrs %>%
...
## change relative to threshold from observation values ----
trendyrs <- min_year:(min_year + regr_length)
lapply(
  list(list("chla_offshore", "Subbasin"), list("chla_coastal", "Subbasin")),
  function(x){
    chla_lm_estim <- get(x[[1]]) %>% 
      filter(year %in% trendyrs) %>% 
      group_by(!!!syms(x[[2]])) %>% 
      mutate(trendcalc_nyrs = n_distinct(year), num_obs = n()) %>% 
      ## how much data is enough data to calculate short-term trends?
      ...
      left_join(select(eut_thresholds, Subbasin = basin, threshold = summer_chla)) %>% 
      mutate(
        ## change w.r.t. thresholds...
        expectedchange5yrs = coef(trend_mdl)["year"]*future_year,
        score = -expectedchange5yrs/threshold,
        ## plotting variables, to visualize
        current = predict(trend_mdl, data.frame(year = max(trendyrs))),
        prj = predict(trend_mdl, data.frame(year = max(trendyrs) + future_year))
      )
    
    p <- ggplot(left_join(filter(get(x[[1]]), year %in% trendyrs), select(chla_lm_estim, Subbasin, current, prj))) +
      geom_point(aes(x = year, y = chla_ug_l), size = 0.2, alpha = 0.4) +
      geom_vline(xintercept = max(trendyrs), size = 0.2) +
      geom_vline(xintercept = max(trendyrs) + future_year, size = 0.2) +
      geom_hline(aes(yintercept = prj), color = "tomato") +
      geom_hline(aes(yintercept = current), alpha = 0.3) +
      facet_wrap(vars(!!!syms(x[[2]])), scales = "free") +
      labs(x = NULL, y = NULL)
    assign(sprintf("%s_lm_plot_%s", x[[1]], stringr::str_to_lower(x[[2]])), p, envir = .GlobalEnv)
    createdplots <- sprintf("%s_lm_plot_%s", x[[1]], stringr::str_to_lower(x[[2]]))
  }
)
chla_offshore_lm_plot_subbasin
chla_coastal_lm_plot_subbasin

## trend score dataframe ----
## assign basin trend to BHI regions
chla_trend <- chla_trend %>% 
  right_join(basin_lookup, by = "subbasin") %>% 
  mutate(
    score = round(trend_score, 3),
    region = rgn_nam,
...
```{r eut nutrients map of raw data points, results = "show", fig.width = 9.5, fig.height = 7}
## make basemap
...
basemap +
  ...
  geom_sf(data = nutrients_obs_mapdf, aes(color = coastal), size = 1.2, alpha = 0.3) +
  scale_color_manual(values = c("darkseagreen", "salmon")) +
  labs(color = "Offshore or Coastal Measurement") +
  theme_bw() +
  theme(legend.position = c(0.1, 0.9), legend.background = element_rect(fill = "ghostwhite"))
```

```{r nutrients data check coastal versus offshore, results = "show", fig.width = 11}
...
plotdf <- nut_rgns %>% 
  ...
  filter(!(is.na(Subbasin) & is.na(nutrient)))

for(r in c("Subbasin", "BHI_ID")){
  ## timeseries plots
  plottimeseries <- ggplot(plotdf) +
    geom_point(aes(winterofyear, value, color = coastal), size = 0.5, alpha = 0.3) +
    labs(x = "Winter of Year", y = NULL, color = "Coastal/Offshore") +
    scale_color_manual(values = c("darkseagreen", "salmon")) +
    facet_wrap(c("nutrient", r), scales = "free_y") +
    theme(axis.text.x = element_text(size = 7, angle = 90))
  assign(sprintf("plot_nut_%s_ts", stringr::str_to_lower(r)), plottimeseries, envir = .GlobalEnv)
  
  ## boxplots comparing coastal/offshore by region or subbasin
  plotboxplot <- ggplot(mutate(plotdf, month = as.factor(month))) +
    geom_boxplot(aes(month, value, fill = coastal), size = 0.2, outlier.size = 0.3, alpha = 0.4) +
    labs(x = "Month", y = NULL, fill = "Coastal/Offshore") +
    scale_fill_manual(values = c("darkseagreen", "salmon")) +
    facet_wrap(c("nutrient", r), scales = "free_y")
    assign(sprintf("plot_nut_%s_box", stringr::str_to_lower(r)), plotboxplot, envir = .GlobalEnv)
}

gridExtra::grid.arrange(plot_nut_subbasin_ts, plot_nut_subbasin_box, nrow = 1)
...
#### 3.3.3 Status Calculaion

**Explore status years**

```{r what are the last yeasr in nutrients subbasin means dataframe, results = "hide"}
## get the last year of non-NA data
nut_last_year <- subbasin_target_nut %>%
  filter(!is.na(nut_mean), coastal == "Both Combined") %>%
  group_by(Subbasin, nutrient) %>%
  summarise(last_year = last(winterofyear)) %>%
  tidyr::pivot_wider(names_from = nutrient, values_from = last_year, names_prefix = "last_year_") %>% 
  print(n = 15)
```
...
```{r eut nutrients status calculation, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 8}
...
ggplot(mutate(nut_status, indicator = ifelse(indicator  == "dip", "DIP", "DIN"))) +
  geom_col(
    aes(region, score, fill = subbasin),
    position = position_dodge(), 
    color = "grey", 
...
nut_status <- nut_status %>% filter(coastal == "Both Combined") %>% select(-coastal)
```

<br>

#### 3.3.4 Trend Calculation {-}

```{r eut nutrients indicators trend calculation, results = "hide", echo = TRUE, fig.width = 9.5}
## basin trend regression ----
nut_trend <- nut_status_allyrs %>%
  filter(coastal == "Both Combined") %>% 
  distinct(subbasin, winterofyear, nutrient, status) %>%
  group_by(subbasin, nutrient) %>%
  ## calculate trend only if:
  ## have at least X yrs data (min_regr_length) in the last Y yrs (regr_length) of time series
  do(tail(., n = regr_length)) %>%  
  do({
    if(sum(!is.na(.$status)) >= min_regr_length){
      data.frame(trend_score = max(-1, min(1, coef(lm(status ~ winterofyear, .))["winterofyear"]*future_year)))
    } else {
      data.frame(trend_score = NA)
    }
  }) %>%
...
lapply(
  list(
    list("Offshore", "DIN"), list("Coastal", "DIN"),
    list("Offshore", "DIP"), list("Coastal", "DIP")
  ),
  function(x){
    nut_lm_estim <- nut_rgns %>% 
      filter(winterofyear %in% trendyrs, coastal == x[[1]], nutrient == x[[2]]) %>% 
      group_by(Subbasin) %>% 
      mutate(trendcalc_nyrs = n_distinct(winterofyear), num_obs = n()) %>% 
      ## how much data is enough data to calculate short-term trends?
      filter((trendcalc_nyrs >= 6 & num_obs > 18)||trendcalc_nyrs > 8) %>%
      do(trend_mdl = lm(value ~ winterofyear, data = .)) %>% 
      left_join(select(eut_thresholds, Subbasin = basin, threshold = ends_with(x[[2]]))) %>% 
      mutate(
        ## change w.r.t. thresholds...
    ...
    plotdf <- nut_rgns %>% 
      filter(winterofyear %in% trendyrs, coastal == x[[1]], nutrient == x[[2]]) %>% 
      left_join(select(nut_lm_estim, Subbasin, current, prj))
    
    p <- ggplot(plotdf) +
      geom_point(aes(x = winterofyear, y = value), size = 0.2, alpha = 0.4) +
      geom_vline(xintercept = max(trendyrs), size = 0.2) +
      geom_vline(xintercept = max(trendyrs) + future_year, size = 0.2) +
      geom_hline(aes(yintercept = prj), color = "tomato") +
      geom_hline(aes(yintercept = current), alpha = 0.3) +
    ...
    assign(stringr::str_to_lower(sprintf("%s_lm_plot_%s", x[[1]], x[[2]])), p, envir = .GlobalEnv)
    createdplots <- stringr::str_to_lower(sprintf("%s_lm_plot_%s", x[[1]], x[[2]]))
  }
)
offshore_lm_plot_din
coastal_lm_plot_din
offshore_lm_plot_dip
coastal_lm_plot_dip
...
nut_trend <- nut_trend %>% 
  ...
  mutate(
    ...
    region = rgn_nam,
    dimension = "trend",
    nutrient = stringr::str_to_lower(nutrient)
  ) %>% 
  rename(indicator = nutrient) %>% 
  select(-trend_score)
```
...
```{r timeseries plot of basin eut nutrients status, results = "show", echo = TRUE, fig.width = 9.5}
...
```{r nutrient status and trend for regions plot, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 6}
...
```{r basemap and color palletes for status and trend maps}
basemap <- ggplot2::ggplot(rnaturalearth::ne_countries(scale = "medium", returnclass = "sf")) + 
  geom_sf(size = 0.1, color = "burlywood", alpha = 0.4) +
  scale_x_continuous(limit = c(4, 32)) +
  scale_y_continuous(limit = c(53.5, 66)) +
...
```{r nutrients status and trend maps, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 4}
mapdf_nut <- left_join(
  bhi_rgns_simple %>% 
    mutate(Subbasin = as.character(Subbasin)),
...
gridExtra::grid.arrange(status_din_map, trend_din_map, nrow = 1)
gridExtra::grid.arrange(status_dip_map, trend_dip_map, nrow = 1)
```
