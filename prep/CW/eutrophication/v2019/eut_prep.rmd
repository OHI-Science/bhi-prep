...
```{r secchi timeseries yearly and monthly, results = "show", echo = TRUE, fig.width = 11, fig.height = 10}
...
ggplot(secchi_status %>% left_join(secchi_last_year, by = c("subbasin" = "Subbasin"))) +
  ...
  geom_text(aes(region, score, label = last_year), color = "slategray") +
  coord_flip() +
  labs(x = NULL, y = NULL) +
  theme_bw() +
...
```{r eut chla status calculation, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 4}
...
```{r eut chl a trend calculation, results = "show", warning = FALSE, fig.width = 9, fig.height = 5}
trendyrs <- min_year:(min_year + regr_length)
...
gridExtra::grid.arrange(
  ...
  layout_matrix = rbind(c(1, 2), c(1, 2), c(1, 2), c(1, 2), c(1, NA))
)
...
plottitle <- "with 5 Years in Future and Current Estimates (Red and Blue lines Respectively)"
gridExtra::grid.arrange(
  maketrendplot(filter(trendplotdf, coastal == "Coastal", nutrient == "DIN")) + 
    ggtitle(paste("Coastal DIN", plottitle)),
  maketrendplot(filter(trendplotdf, coastal == "Offshore", nutrient == "DIN")) +
    ggtitle(paste("Offshore DIN", plottitle)),
  maketrendplot(filter(trendplotdf, coastal == "Coastal", nutrient == "DIP")) + 
    ggtitle(paste("Coastal DIP", plottitle)),
  maketrendplot(filter(trendplotdf, coastal == "Offshore", nutrient == "DIP")) + 
    ggtitle(paste("Offshore DIP", plottitle)),
   layout_matrix = matrix(c(rep(c(1,2),4),NA,2, rep(c(3,4),4),NA,4), ncol = 2, byrow = TRUE)
)
