...
secchi_rgns_tmp <- join_rgns_info(
...
secchi_rgns <- sf::st_as_sf(
  secchi_rgns_tmp,
  crs = 4326,
  coords = c("lon","lat"),
  remove = FALSE) %>%  
  sf::st_join(sf::st_transform(offshore_codes, 4326)) %>% 
  mutate(Subbasin = str_replace(Subbasin, "Åland", "Aland"))
st_geometry(secchi_rgns) <- NULL
...
ggplot(subbasin_target_chla, aes(year, mean_chla, group = 1)) + 
  geom_col() +
  geom_line(aes(year, mean_chla), colour = "blue") +
  geom_line(aes(year, target_chla), colour = "red") +
  facet_wrap(~Subbasin)+
  scale_y_continuous(limits = c(0,10)) +
  ggtitle("Mean Summer Chl a against Targe values (coastal)")
...
ggplot(chla_bhi_status, aes(region, score, fill = Subbasin)) +
  ...
  theme_bw() +
  theme(axis.text.y = element_text(size = 7)) +
      ggtitle("Chl a Status by Subbasins (coastal)")
