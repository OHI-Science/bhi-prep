...
offshore_codes <-sf::st_read(file.path(dir_B, "Goals", "CW", "EUT", "HELCOM_subbasins_with_coastal_and_offshore_division_2018", "HELCOM_subbasins_with_coastal_and_offshore_division_2018.shp"))
# st_geometry(coast_codes) <- NULL
...
secchi_rgns_tmp <- sf::st_as_sf(
  combined_secchi_rawdata,
  crs = 4326,
  coords = c("lon","lat"),
  remove = FALSE) %>%  
  sf::st_join(sf::st_transform(offshore_codes, 4326)) 
st_geometry(secchi_rgns_tmp) <- NULL

secchi_rgns_tmp <- secchi_rgns_tmp %>% 
  select(-Subbasin, -HELCOM_ID)

# assign bhi basins to secchi data 
secchi_rgns <- join_rgns_info(
  secchi_rgns_tmp, helcomID_col = "helcom_id", country_col = "country", 
...
ggmap(map) +
  geom_point(aes(x = lon, y = lat), data = loc_no_coastcode_nobhi, 
             size = 0.5)
...
ggmap(map) +
  geom_point(aes(x = lon, y = lat), data = loc_no_coastcode_bhi, 
             size = 0.5)
### many coastal location

## Filter data that are only offshore, coast_code == 0
secchi_offshore <- secchi_rgns %>% 
  filter(helcom_coastal_code == 0) 

dim(secchi_offshore) # 8015

ggmap(map) +
   geom_point(aes(x = lon, y = lat), data = secchi_offshore, colour = "red", size = 1)
...
ggplot(secchi_subbasin_mean_months_summer, aes(year, mean_secchi, group =1)) + 
  geom_point() +
  geom_line() +
  facet_wrap(~Subbasin)+
...
# Plot summer mean secchi
ggplot(secchi_bhi_mean_months_summer, aes(year, mean_secchi, group = 1)) + 
  geom_point() +
  geom_line() +
  facet_wrap(~BHI_ID)+
  scale_y_continuous(limits = c(0,10)) +
...
ggplot(subbasin_target_secchi, aes(year, mean_secchi, group = 1)) + 
  geom_point() +
  geom_line() +
  geom_line(aes(year, target_secchi), colour = "red") +
  facet_wrap(~Subbasin)+
  scale_y_continuous(limits = c(0,10)) +
  ggtitle("Mean Summer Secchi against Targe values (offshore)") +
...
subbasin_target_secchi <- subbasin_target_secchi %>% 
  filter(year >=2010)
...
**METHOD 2 (from each obs value -Ellie-)**
```{r trend calculation}
## Calculate basin trend
...
# # assign helcom offshore codes
chla_rgns_tmp <- sf::st_as_sf(
  chla_rawdata,
  crs = 4326,
  coords = c("lon","lat"),
  remove = FALSE) %>%  
  sf::st_join(sf::st_transform(offshore_codes, 4326)) 
st_geometry(chla_rgns_tmp) <- NULL
...
## 441 observations with no coast_code or BHI_ID
...
dim(chla_offshore) # 4879

ggmap(map) +
   geom_point(aes(x = lon, y = lat), data = chla_offshore, size = 0.5)
...
# Calculate mean monthly value for each summer month (subbasin)
chla_subbasin_mean_months <- chla_coast %>% 
...
# Calculate mean monthly value for each summer month (bhi basins)
chla_bhi_mean_months <- chla_coast %>% 
...
ggplot(subbasin_target_chla, aes(year, mean_chla, group = 1)) + 
  ...
  facet_wrap(~Subbasin)+
  scale_y_continuous(limits = c(0,10)) +
  ggtitle("Mean Summer Chl a against Targe values (whole)")
...
Only two basins have data for 2019 (Bothnian Bay and Kattegat), which will be used as the status year, while seven (7) have data for 2018 and the remaining four (4) basins: 
Bay of Mecklenburg (2010), Great Belt (2014), Kiel Bay (2010), and The Quark (2013).
...
subbasin_target_chla <- subbasin_target_chla2 %>% 
...
ggplot(chla_bhi_status, aes(region, score, fill = Subbasin)) +
  ...
  theme_bw() +
  theme(axis.text.y = element_text(size = 7)) +
      ggtitle("Chl a Status by Subbasins (coastal) [average 2010-2020]")
