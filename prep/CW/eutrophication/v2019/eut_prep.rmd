...
## 5604 observations with no coast_code or BHI_ID
loc_no_coastcode_nobhi <- secchi %>% 
  filter(is.na(HELCOM_COASTAL_CODE) & is.na(BHI_ID))%>%
...
secchi %>% 
  ...
  distinct(.) # one region is offshore
...
secchi_offshore <- secchi %>% 
  filter(HELCOM_COASTAL_CODE == 0)

dim(secchi_offshore) # 17182   14 
