...
subbasin_mean_months <- secchi_rgns_shp %>% 
...
subbasin_mean_months_summer <- subbasin_mean_months %>% 
...
ggplot(subbasin_mean_months_summer) + 
...
subbasin_target_secchi <- left_join(sub_mean_months_summer, target_secchi, by = "subbasin")
...
last_year = subbasin_target_secchi %>%
...
subbasin_trend <- subbasin_status %>%
  group_by(subbasin) %>%
...
bhi_trend <- left_join(basin_lookup, subbasin_trend, by = "subbasin") %>%
...
subbasin_trend_secchi <- lm_estim %>% 
  ungroup() %>% 
  select(-trend_mdl) %>% 
  group_by(subbasin) %>% 
...
bhi_trend <- left_join(basin_lookup, subbasin_trend_secchi, by = "subbasin") %>%
...
basin monthly mean = mean of all samples within month and subbasin
```{r calculate monthly mean secchi}
subbasin_mean_months_chla <- chla_rgns_shp %>% 
...
## Plot mean monthly value by subbasin
ggplot(subbasin_mean_months_chla) + 
...
subbasin_mean_months_summer_chla <- subbasin_mean_months_chla %>% 
...
# Plot summer mean secchi
ggplot(subbasin_mean_months_summer_chla) + 
...
bhi_mean_months_chla <- chla_rgns_shp %>% 
...
basin summer mean = mean of basin monthly mean values
```{r calculate mean summer chl a}
bhi_mean_months_summer_chla <- bhi_mean_months_chla %>% 
...
# Plot summer mean secchi
ggplot(bhi_mean_months_summer_chla) + 
  geom_point(aes(year, mean_chla)) +
  geom_line(aes(year, mean_chla)) +
  facet_wrap(~bhi_id)+
  scale_y_continuous(limits = c(0,10)) +
...
subbasin_target_chla <- left_join(subbasin_mean_months_summer_chla, target_chla, by = "subbasin")
...
## get the last year of non-NA data
last_year_chla <- subbasin_target_chla %>%
...
##which are not in 2013
last_year_chla %>% filter(last_year < 2018)
...
subbasin_status_chla <- subbasin_target_chla %>%
  mutate(., status =  pmin(1, mean_chla/target_chla)) %>%
  dplyr::select(subbasin, year, status) 

## Assign basin status to BHI regions

bhi_status_chla <- subbasin_status_chla %>%
  group_by(subbasin) %>%
  filter(year == max(year)) %>% #select last year of data for status in each basin (this means status year differs by basin)
  mutate(status = round(status*100)) %>% #status is whole number 0-100
  ungroup() %>%
...
```{r trend calculation}
## Calculate basin trend
subbasin_trend_chla1 <- subbasin_status_chla %>%
  group_by(subbasin) %>%
  do(tail(. , n = regr_length)) %>%  # calculate trend only if there is at least X years of data (min_regr_length) in the last Y years of time serie (regr_length)
  do({if(sum(!is.na(.$status)) >= min_regr_length)
    data.frame(trend_score = 
...
## Assign basin trend to BHI regions
bhi_trend_chla1 <- left_join(basin_lookup, subbasin_trend_chla1, by = "subbasin") %>%
  mutate(score = round(trend_score, 2),
         dimension = "trend") %>%
  dplyr::select(bhi_id, dimension, score )
...
subbasin_trend_chla2 <- lm_estim_chla %>% 
  ungroup() %>% 
  select(-trend_mdl) %>% 
  group_by(subbasin) %>% 
  summarize(trend = mean(trend_score, na.rm = TRUE) %>% round(3)) %>%
...
bhi_trend_chla2 <- left_join(basin_lookup, subbasin_trend_chla2, by = "subbasin") %>%
  mutate(score = round(trend, 2),
         dimension = "trend") %>%
  dplyr::select(bhi_id, dimension, score )
...
```{r bhi status and trend plot}
# Method 1
ggplot(rbind(bhi_status_chla, bhi_trend_chla1)) + 
  geom_point(aes(bhi_id, score),size=2)+
  facet_wrap(~dimension, scales="free_y")+
  xlab("BHI region")

# Method 2
ggplot(rbind(bhi_status_chla, bhi_trend_chla2)) + 
  geom_point(aes(bhi_id, score),size=2)+
  facet_wrap(~dimension, scales="free_y")+
  xlab("BHI region")
```
