...
ggplot(sub_mean_months) + 
  geom_point(aes(year, mean_secchi, colour = factor(month)), size = 1.5, alpha = 0.5) +
  geom_line(aes(year, mean_secchi, colour = factor (month)), alpha = 0.5) +
  facet_wrap (~subbasin) +
...
sub_target_secchi <- left_join(sub_mean_months_summer, target_secchi, by = "subbasin")

ggplot(sub_target_secchi) + 
  geom_point(aes(year, mean_secchi)) +
...
basin_status <- sub_target_secchi %>%
...
**Spatial Visualization of chlorophyll a data**
```{r}
# Plot by bhi regions
...
#### 3.3.2 Mean Chl a Calculation

**Calculate mean monthly value for each summer month (subbasin)** 
basin monthly mean = mean of all samples within month and subbasin
```{r calculate monthly mean chl a}
sub_mean_chla_months <- chla_rgns_shp %>% 
...
## Plot mean monthly value by subbasin
ggplot(sub_mean_chla_months) + 
  geom_point(aes(year, mean_chla, colour = factor(month)), size = 1.5, alpha = 0.5) +
  geom_line(aes(year, mean_chla, colour = factor (month)), alpha = 0.5) +
  facet_wrap (~subbasin) +
  scale_y_continuous(limits = c(0, 10)) +
  ggtitle("Mean Monthly Chl a")
...
basin summer mean = mean of basin monthly mean values
```{r calculate mean summer chl a}
sub_mean_chla_months_summer <- sub_mean_chla_months %>% 
...
# Plot summer mean secchi
ggplot(sub_mean_chla_months_summer) + 
...
basin monthly mean = mean of all samples within month and bhi basins
```{r calculate monthly mean chl a}
bhi_mean_chla_months <- chla_rgns_shp %>% 
...
basin summer mean = mean of basin monthly mean values
```{r calculate mean summer chl a}
bhi_mean_chla_months_summer <- bhi_mean_chla_months %>% 
  dplyr::select(year, bhi_id, mean_chla) %>%
  group_by(year, bhi_id) %>%
  summarise(mean_chla = round(mean(mean_chla,na.rm = TRUE), 1)) %>%
  ungroup()  #in mean calculation all some months to have NA, ignore for that years calculation

# Plot summer mean chl a
ggplot(bhi_mean_chla_months_summer) + 
  geom_point(aes(year, mean_chla)) +
  geom_line(aes(year, mean_chla)) +
  facet_wrap(~bhi_id)+
  scale_y_continuous(limits = c(0, 10)) +
...
**Plot summer chla with target values indicated**
Horizontal lines are HELCOM target values 
```{r summer secchi with target }
# chla thresholds
target_chla <- eut_thresholds %>% 
  dplyr::select(subbasin = basin, target_chla = summer_chla) 
sub_target_chla <- left_join(sub_mean_chla_months_summer, target_chla, by = "subbasin")

ggplot(sub_target_chla) + 
  geom_point(aes(year, mean_chla)) +
  geom_line(aes(year, target_chla)) +
  facet_wrap(~subbasin) +
  scale_y_continuous(limits = c(0,10)) +
  ggtitle("Mean Summer Chl a against Target values")
```
<br/>

#### 3.3.3 Status Calculation

##### 3.3.3.1 Explore status years
...
## which are not in 2019
last_year %>% filter(last_year < 2018)
```

##### 3.3.3.2 Calculate status for each subbasin

**Status calculation with raw (non-modeled) mean summer chl a by subbasin** 
...
## Calculate basin status = basin_mean/basin_target
sub_status_chla <- sub_target_chla %>%
  mutate(., status =  pmin(1, mean_chla/target_chla)) %>%
  dplyr::select(subbasin, year, status) 

## Assign basin status to BHI regions
bhi_status_chla <- sub_status_chla %>%
  group_by(subbasin) %>%
  filter(year == max(year)) %>% #select last year of data for status in each basin (this means status year differs by basin)
  mutate(status = round(status*100)) %>% #status is whole number 0-100
  ungroup() %>%
...
##### 3.3.3.3 Calculate Trend 

```{r trend calculation}
## Calculate basin trend
sub_trend_chla <- sub_status_chla %>%
  group_by(subbasin) %>%
  do(tail(. , n = regr_length)) %>%  # calculate trend only if there is at least X years of data (min_regr_length) in the last Y years of time serie (regr_length)
  do({if(sum(!is.na(.$status)) >= min_regr_length)
    data.frame(trend_score = 
...
## Assign basin trend to BHI regions
bhi_trend_chla <- left_join(basin_lookup, sub_trend_chla, by = "subbasin") %>%
  mutate(score = round(trend_score,2),
         dimension = "trend")%>%
  dplyr::select(bhi_id, dimension, score )
```
...
##### 3.3.3.4 Plot Status and Trend

**Plot basin status**
Basin status is initially a value between 0 - 1.  Calculated for each year between 2000 and 2018.  
```{r plot basin status}
ggplot(sub_status_chla) + 
  geom_point((aes(year, status))) +
  facet_wrap(~subbasin) +
  theme(axis.text.x = element_text(colour="grey20", size=8, angle=90, 
                                   hjust=.5, vjust=.5, face = "plain"))
...
Status values can range from 0-100 -- this is the status for the *most recent* year. In most cases this is 2019. No status or trend for 5 or 6 - these are The Sound, which had no data. 
```{r bhi status and trend plot}
ggplot(rbind(bhi_status_chla, bhi_trend_chla)) + 
  geom_point(aes(bhi_id, score), size=2) +
  facet_wrap(~dimension, scales = "free_y") +
  xlab("BHI region")
```
