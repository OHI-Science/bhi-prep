...
chla_whole_spatial_codeareas <- basemap +
  geom_sf(
    mapping = aes(fill = Name),
    data = rmapshaper::ms_simplify(input = coastal_codes) %>% sf::st_as_sf(),
...
```{r chla offshore coastal data maps, results = "show", echo = FALSE, fig.width = 12, fig.height = 5}
gridExtra::grid.arrange(
  chla_offshore_spatial + ggtitle("Offshore Chl a. Data"), 
  chla_coast_spatial + ggtitle("Coastal Chl a. Data"), 
  chla_whole_spatial + ggtitle("All Chl a. Data"),
...
chla_summer_months_mean <- chla_summer_months_mean %>% 
  ...
  summarize(
    mean_chla = weighted.mean(mean_chla, area_km2_wfd, na.rm = TRUE),
    chla_sd = sqrt(weighted.mean((chla_sd^2), area_km2_wfd, na.rm = TRUE)),
    n_pts = sum(n_pts)
  ) %>% 
  mutate(coastal = "All Combined", code = "Combined") %>% 
  ungroup() %>% 
...
wfd_area_target_chla <- file.path(dir_prep, "supplement", "lookup_tabs", "rgns_complete.csv") %>% 
  read_csv() %>% 
  select(basin = subbasin, code = helcom_id) %>% 
  distinct() %>% 
  left_join(eut_thresholds, by = "basin") %>% 
  select(code, target_chla = summer_chla) %>% 
...
chla_meansplot <- wfd_area_target_chla %>% 
  filter(code != "Combined") %>% 
  ggplot(aes(year, mean_chla, color = code)) +  
  geom_errorbar(
  ...
  geom_line(aes(year, ma5yr), size = 0.2, show.legend = FALSE) + 
...
chla_last_year <- wfd_area_target_chla %>%
  filter(!is.na(mean_chla), code != "Combined") %>%
