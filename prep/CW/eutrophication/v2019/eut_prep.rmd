...
chla_status_allyrs <- subbasin_target_chla %>%
  ...
  mutate(region = rgn_nam)

## select last year of data for status in each basin, this means status year differs by basin
chla_status <- chla_status_allyrs %>%
  group_by(subbasin) %>%
  filter(year == max(year)) %>% 
  mutate(status = round(status*100, 2)) %>% # keep decimal places, will round when combine eut indicators
  ungroup %>% 
  mutate(dimension = "status", indicator = "chla") %>% 
  dplyr::select(Subbasin = subbasin, BHI_ID = bhi_id, dimension, score = status, region, indicator, coastal)

## Alternative status calculation: Status calculated using BHI region means
# chla_status_allyrs <- bhi_id_target_chla %>%
#   rename(subbasin = Subbasin, bhi_id = BHI_ID) %>%
#   mutate(status = pmin(1, target_chla/mean_chla)) %>%
#   ## or use five year rolling means rather than annual mean?
#   # mutate(status = pmin(1, target_chla/ma5yr)) %>%
...
ggplot(mutate(chla_status, region = as.factor(region))) +
  geom_col(
    aes(region, score, fill = Subbasin),
    position = position_dodge(), 
    color = "grey", 
...
chla_status <- chla_status %>% filter(coastal == "Both")
```
...
chla_trend <- chla_status_allyrs %>%
  ...
  do({
    if(sum(!is.na(.$status)) >= min_regr_length){
      data.frame(score = max(-1, min(1, coef(lm(status ~ year, .))["year"]*future_year)))
    } else {
      data.frame(score = NA)
    }
  }) %>%
...
chla_trend <- chla_trend %>% 
  right_join(basin_lookup, by = "subbasin") %>% 
  mutate(dimension = "trend", indicator = "chla") %>%
  dplyr::select(Subbasin = subbasin, BHI_ID = bhi_id, region = rgn_nam, dimension, score, indicator)
```
...
nut_rgns <- nut_rgns_shp %>% 
  ...
  filter(year >= 2000) %>% 
  mutate(coastal = ifelse(is.na(helcom_coastal_code), "Coastal", "Offshore")) %>% 
  select(latitude, longitude, Subbasin, BHI_ID, coastal, year, month, time, depth_m, din, dip) %>% 
...
expandcases <- tidyr::expand(nut_rgns, tidyr::nesting(Subbasin, BHI_ID, year, month, nutrient))
plotdf <- nut_rgns %>% 
...
for(r in c("Subbasin", "BHI_ID")){
  ...
  plottimeseries <- ggplot(plotdf) +
    geom_point(aes(year, value, color = coastal), size = 0.5, alpha = 0.3) +
    labs(x = "Year", y = NULL, color = "Coastal/Offshore") +
    scale_color_manual(values = c("darkseagreen", "salmon")) +
...
for(r in c("Subbasin", "BHI_ID")){
  ...
  nutrient_monthly_means <- nut_rgns %>% 
    ...
    group_by(year, month, coastal, nutrient, Subbasin, !!!syms(r)) %>%
    summarise(
  ...
  nutrient_winter_months_mean <- nutrient_monthly_means %>% 
    group_by(year, coastal, nutrient, Subbasin, !!!syms(r)) %>%
    summarise(
  ...
  ## calculate min and max, and rolling 5 year mean
  ## bind to coastal and offshore rows
  nutrient_winter_months_mean <- nutrient_winter_months_mean %>% 
    group_by(year, nutrient, Subbasin, !!!syms(r)) %>% 
    summarize(
      nut_mean = mean(nut_mean, na.rm = TRUE),
      nut_sd = sqrt(mean((nut_sd^2), na.rm = TRUE)),
      n_pts = sum(n_pts)
    ...
    mutate(coastal = "Both") %>% 
    ungroup() %>% 
    bind_rows(nutrient_winter_months_mean) %>% 
    arrange(!!!syms(r), nutrient, coastal, year) %>% 
  ...
  ## join with basin-specific eutrophication chlorophyl a targets
  nutrient_winter_months_mean <- eut_thresholds %>% 
    select(Subbasin = basin, DIN = winter_DIN, DIP = winter_DIP) %>% 
    tidyr::pivot_longer(c("DIN", "DIP"), names_to = "nutrient", values_to = "target") %>% 
    right_join(nutrient_winter_months_mean, by = c("Subbasin", "nutrient"))
  ## make subbasin_target_nut and bhi_id_target_nut dataframes
  assign(paste0(str_to_lower(r), "_target_nut"), nutrient_winter_months_mean)
  
  ## Plot winter mean nutrients
  for(n in c("DIN", "DIP")){
    meansplot <- ggplot(filter(nutrient_winter_months_mean, nutrient == n), aes(year, nut_mean, group = 1)) + 
      geom_errorbar(
        aes(x = year, ymin = nut_min, ymax = nut_max), 
        alpha = 0.6, size = 0.2, width = 0.2
      ) +
      geom_point() +
      # geom_line(aes(year, ma5yr), size = 0.2) +
    ...
    if(n == "DIN"){
      meansplot <- meansplot + scale_y_continuous(limits = c(0, 25))
    } else {
      meansplot <- meansplot + scale_y_continuous(limits = c(0, 2))
    }
...
```{r what is the last year in nutrients subbasin means dataframe, results = "hide"}
...
```{r eut nutrients indicator status calculation, results = "show", echo = TRUE, fig.width = 9.5}
...
```{r eut nutrients indicator trend calculation, results = "hide", echo = TRUE, fig.width = 9.5}
...
```{r nutrient status and trend for regions plot, results = "show", echo = TRUE, fig.width = 9.5, fig.height = 4}
ggplot(rbind(nutrient_status, nutrient_trend)) + 
  geom_col(aes(region, score, fill = subbasin), alpha = 0.5, show.legend = FALSE) +
  coord_flip() +
  facet_wrap(~dimension, scales = "free_x", nrow = 1) +
  labs(x = "BHI Region ID\n", y = NULL) +
