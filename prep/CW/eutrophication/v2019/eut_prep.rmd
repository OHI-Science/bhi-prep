...
chla_rgns_shp <- join_rgns_info(
  chla, helcomID_col = "helcom_id", country_col = "country", 
  latlon_vars = c("^lat", "^lon"), return_spatial = FALSE, 
  rgn_shps_loc = file.path(dirname(dir_B), "Shapefile"), buffer_shp = NULL) %>% 
  # select summer months (June-September) and Years >= 2000  
  filter(month == "06" | month == "07" | month == "08" | month == "09") %>% 
...
ggplot(sub_mean_chla_months) + 
  geom_point(aes(year, mean_chla, colour = factor(month)), size = 1.2, alpha = 0.4) +
  geom_line(aes(year, mean_chla, colour = factor (month))) +
  facet_wrap (~subbasin) +
  scale_y_continuous(limits = c(0, 10)) +
...
ggplot(bhi_mean_chla_months) + 
  geom_point(aes(year, mean_chla, colour = factor(month)), size = 1.2, alpha = 0.4) +
  geom_line(aes(year, mean_chla, colour = factor(month)), alpha = 0.3) +
  facet_wrap (~bhi_id) +
  scale_y_continuous(limits = c(0,10)) +
  ggtitle("Mean Monthly Chl a")
