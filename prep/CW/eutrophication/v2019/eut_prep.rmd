...
## 608 observations with no coast_code or BHI_ID
loc_no_coastcode_nobhi <- secchi_rgns_shp %>% 
  filter(is.na(helcom_coastal_code) & is.na(BHI_ID))%>%
...
map <- get_map(location = c(8.5, 53, 32, 67.5))
 ...
 plot_map1 <- ggmap(map) +
   geom_point(aes(x = longitude, y = latitude), data = loc_no_coastcode_nobhi, size = 0.5)
 plot_map1
## these locations are very coastal or outside of the Baltic Sea

### observations with NA for the coast_code but have BHI_ID
loc_no_coastcode_bhi <- secchi_rgns_shp %>% 
  filter(is.na(helcom_coastal_code) & !is.na(BHI_ID)) %>% 
  dplyr::select(latitude, longitude) %>%
  distinct()

 plot_map2 <- ggmap(map) +
   geom_point(aes(x = longitude, y = latitude), data = loc_no_coastcode_bhi, size = 1.5)
plot_map2
## some of them are not coastal stations

## What are coastal codes for The Sound (BHI regions 5,6)
## Region 6
secchi_rgns_shp %>% 
  filter(BHI_ID %in% 6) %>% 
  dplyr::select(BHI_ID, year, date, latitude, longitude, helcom_coastal_code, supplier) %>%
  arrange(desc(year)) %>% 
  distinct(.)

secchi_rgns_shp %>% 
  filter(BHI_ID %in% 6) %>% 
  dplyr::select(helcom_coastal_code, supplier) %>% 
  distinct(.) # no region offshore
...
secchi_rgns_shp %>% 
  filter(BHI_ID %in% 5) %>% 
  dplyr::select(helcom_coastal_code, supplier) %>% 
  distinct(.) ## one region has a coastal code, two have NA

secchi_thesound <- secchi_rgns_shp %>% 
  filter(BHI_ID == 5 | BHI_ID == 6) %>% 
  dplyr::select(BHI_ID, year, date, latitude, longitude, helcom_coastal_code, supplier) %>% 
  arrange(desc(year)) %>% 
  distinct(.)

ggmap(map) +
   geom_point(aes(x = longitude, y = latitude), data = secchi_thesound, size = 0.5)
## no offshore stations in The Sound?!

## Filter data that are only offshore, coast_code == 0
secchi_offshore <- secchi_rgns_shp %>% 
  filter(helcom_coastal_code == 0)

dim(secchi_offshore) # 4948

offshore <- ggmap(map) +
   geom_point(aes(x = longitude, y = latitude), data = secchi_offshore, size = 0.5)
offshore
...
secchi_coast <- secchi_rgns_shp %>% 
  filter(!(helcom_coastal_code == 0))

dim(secchi_coast) # 8871

coastal_secchi <- ggmap(map) +
   geom_point(aes(x = longitude, y = latitude), data = secchi_coast, size = 0.5)
coastal_secchi
...
```{r calculate monthly mean secchi by subbasins}
# Calculate mean monthly value for each summer month (subbasin)
secchi_subbasin_mean_months <- secchi_offshore %>% 
...
ggplot(secchi_subbasin_mean_months_summer) + 
...
secchi_bhi_mean_months <- secchi_offshore %>% 
...
All basins have data for 2018, which will be used as the status year, except for four basins: 
Aland Sea (2017), Gulf of Riga (2017), The Quark (2017), and Western Gotland Basin (2016).
For these four basins, their most recent year's data is used for current status calculations
...
secchi_last_year %>% filter(last_year < 2018)
...
ggplot(secchi_bhi_status, aes(region, score, fill = Subbasin)) +
...
## 258 observations with no coast_code or BHI_ID
loc_no_coastcode_nobhi <- chla_rgns_shp %>% 
  filter(is.na(helcom_coastal_code) & is.na(BHI_ID))%>%
  dplyr::select(latitude, longitude)%>%
  distinct() # 258
...
### observations with NA for the coast_code but have BHI_ID
loc_no_coastcode_bhi <- chla_rgns_shp %>% 
  filter(is.na(helcom_coastal_code) & !is.na(BHI_ID)) %>% 
  dplyr::select(latitude, longitude) %>%
  distinct()

 plot_map2 <- ggmap(map) +
   geom_point(aes(x = longitude, y = latitude), data = loc_no_coastcode_bhi, size = 0.7)
plot_map2
## some of them are not coastal stations
...
chla_rgns_shp %>% 
  filter(BHI_ID %in% 6) %>% 
  dplyr::select(BHI_ID, year, date, latitude, longitude, helcom_coastal_code) %>%
  arrange(desc(year)) %>% 
  distinct(.)
...
chla_rgns_shp %>% 
  filter(BHI_ID %in% 5) %>% 
  dplyr::select(helcom_coastal_code) %>% 
  distinct(.) ## one region has a coastal code, one has NA

chla_thesound <- chla_rgns_shp %>% 
  filter(BHI_ID == 5 | BHI_ID == 6) %>% 
  dplyr::select(BHI_ID, year, date, latitude, longitude, helcom_coastal_code) %>% 
  arrange(desc(year)) %>% 
  distinct(.)
...
ggmap(map) +
   geom_point(aes(x = longitude, y = latitude), data = chla_thesound, size = 0.5)
## no offshore stations in The Sound?!

## Filter data that are only offshore, coast_code == 0
chla_offshore <- chla_rgns_shp %>% 
  filter(helcom_coastal_code == 0)

dim(chla_offshore) # 3628

offshore <- ggmap(map) +
   geom_point(aes(x = longitude, y = latitude), data = chla_offshore, size = 0.5)
offshore
## This is a substantial reduction in the number of observations, and removes the northern regions

## Check the difference with the complete dataset, including coast_code having NA
chla_coast <- chla_rgns_shp %>% 
  filter(!(helcom_coastal_code == 0))

dim(chla_coast) # 8599

coastal_chla <- ggmap(map) +
   geom_point(aes(x = longitude, y = latitude), data = chla_coast, size = 0.5)
coastal_chla
...
ggplot(chla_rgns_shp) + 
  geom_point(aes(year, chla_ug_l, colour = rgn_nam), size = 0.3, alpha = 0.4) +
  facet_wrap(~BHI_ID, scales ="free_y") +
  labs(x = "Year", y = "Chl a (ug/L)") +
  ggtitle("Yearly Chl a by bhi regions")

ggplot(chla_rgns_shp) + 
  geom_point(aes(month, chla_ug_l, colour = rgn_nam), size = 0.3, alpha = 0.4) +
  facet_wrap(~BHI_ID, scales ="free_y") +
