...
for(r in c("Subbasin", "BHI_ID")){
  ...
  secchi_monthly_means <- secchi_offshore %>% 
    ...
    mutate(secchi_sd = sd(secchi_m, na.rm = TRUE)) %>%
...
lapply(
  ...
  function(x){
    ...
    ## evaluate coastal/offshore data
...
# gridExtra::grid.arrange(plot_chla_offshore_bhi_ts, plot_chla_coastal_bhi_ts, nrow = 1)
```
...
for(r in c("Subbasin", "BHI_ID")){
  ...
  ## each month will have two means, coastal and offshore
...
```{r winter nutrients with target, results = "show", echo = TRUE, fig.width = 9.5}
