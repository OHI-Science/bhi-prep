...
secchimeansplotsubbasin + ggtitle("Mean Summer Secchi by Subbasin (June-Sept, Offshore) against Target Values")
secchimeansplotbhi_id + ggtitle("Mean Summer Secchi by BHI Region (June-Sept, Offshore) against Target Values")
```
...
ggplot(left_join(filter(secchi_offshore, year %in% trendyrs), select(secchi_lm_estim, Subbasin, current, prj))) +
  ...
  labs(x = NULL, y = NULL) +
  ggtitle("Offshore Secchi with 5 Years in Future and Current Estimates (Red and Grey lines Respectively)")
...
```{r coastal and offshore chla plots by basin, results = "show", echo = TRUE, fig.width = 12.5, fig.height = 7}
...
lapply(
  ...
  function(x){
    ...
    p <- ggplot(left_join(filter(get(x[[1]]), year %in% trendyrs), select(chla_lm_estim, Subbasin, current, prj))) +
      ...
      labs(x = NULL, y = NULL) +
      ggtitle(paste(
        str_to_title(str_remove(x[[1]], "chla_")), 
        "Chlorophyll a with 5 Years in Future and Current Estimates (Red and Grey lines Respectively)"
      ))
