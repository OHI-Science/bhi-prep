...
map_cwcon_initial <- function(dataset, yr, matrix = NULL){
  ...
  map <- ggplot(df) + 
    ...
    scale_fill_gradient(low = "midnightblue", high = "lightcyan", na.value = "whitesmoke", limits = c(0, 15)) +
    geom_sf(
  ...
  basinplot <- ggplot(filter(df, !is.na(Matrix))) +
    geom_point(
      aes(Subbasin, meanVal, fill = obs_by_matrix, size = numLocation),
...
```{r map spatial dist of PCBs year 2014, results = "show", results = "show", fig.width = 9.5}
pcb2014 <- map_cwcon_initial(pcb_qflag_adjust, yr = 2014, matrix = "both")
...
```{r map spatial dist of PCBs year 2016, results = "show", fig.width = 9.5}
pcb2016 <- map_cwcon_initial(pcb_qflag_adjust, yr = 2016, matrix = "both")
...
pcb2014to2019 <- map_cwcon_initial(pcb_qflag_adjust, yr = 2014, matrix = "both")
...
pcb_qflag_adjust <- cwcon_indicators(pcb_qflag_adjust)
pcb_noqflag <- cwcon_indicators(pcb_noqflag)

pcb_status <- bind_rows(
  pcb_qflag_adjust$basin_status %>% 
    mutate(approach = "Adjusted", spatialunits = "Subbasins"),
  pcb_qflag_adjust$bhirgn_status %>% 
...
```{r pfos qflag adjustment congeners by basin, results = "show", fig.height = 52, fig.width = 9.5}
## pcb qflag adjustment congeners by basin and congener, 
## red x's indicate flagged data, colorscale indicates number of observations
plotdf <- pfos %>% 
...
pfos_basin_qflags <- function(plotdf, subbasin_name = NULL){
  
  if(!is.null(subbasin_name)){
    plotdf <- plotdf %>% filter(Subbasin == subbasin_name)
  }
  plot <- ggplot(plotdf) + 
    scale_color_distiller(palette = "GnBu", direction = -1) +
    ...
    labs(x = NULL, y = "Congener Concentration (ug/kg wet weight) measured in all Species") +
    theme_bw()
...
```{r pfos qflag adjustment congeners by basin}
pfos_basin_qflags(plotdf, )
```
...
cols <- colorRampPalette(fullpal)(42)[sample(1:42, size = 22)]
...
map_cwcon_initial(pfos_qflag_adjust %>% mutate(Matrix = "biota"), yr = 2008:2019)
```
