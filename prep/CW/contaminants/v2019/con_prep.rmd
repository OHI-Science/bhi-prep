...
**PCBs in Biota, Visualize flagged data in Timeseries**
...
**PCBs in Sediment, Visualize flagged data in Timeseries**

```{r pcbs sed qflag adjustment congeners by basin, results = "show", fig.height = 22, fig.width = 9.5}
plotdf <- pcb_sed %>% 
...
ggplot(plotdf) + 
  scale_color_distiller(palette = "GnBu", direction = -1) +
  geom_point(aes(date, value_dry_wgt, color = countObs), show.legend = FALSE, size = 0.6, alpha = 0.4) +
  geom_point(aes(date, value_adj), color = "red", shape = 4, size = 1, alpha = 0.6) +
  facet_wrap(
    c("variable", "Subbasin"), 
    labeller = label_wrap_gen(width = 35, multi_line = FALSE), 
...
```{r congeners timeseries monthly by basin, results = "show", message = FALSE, echo = TRUE, fig.width = 9.5, fig.height = 9}
## create color palettes, 6 colors for number of congeners in the dataset...
cols <- c(
...
plot_cwcon_initial <- function(dataset, col_pal, contam_param = "OC-CB", cntry_or_basin = "subbasin"){
  ...
  plot <- ggplot(data = dat, aes(Date, Value, fill = Congener)) +
    ...
    ggtitle(sprintf("Congeners Monthly Sums by %s", str_to_title(cntry_or_basin)))
  
  return(plotly::ggplotly(plot))
}

cols <- colorRampPalette(cols)(42)[sample(1:42, size = 6)]
plot_cwcon_initial(pcb_bio_qflag_adjust, col_pal = cols, contam_param = "OC-CB", cntry_or_basin = "country")

cols <- colorRampPalette(cols)(42)[sample(1:42, size = 7)]
plot_cwcon_initial(pcb_sed_qflag_adjust, col_pal = cols, contam_param = "OC-CB", cntry_or_basin = "country")
```
...
```{r map spatial dist of PCB bio, results = "show", echo = TRUE, fig.width = 10, fig.height = 9, out.width = "120%"}
...
map_cwcon_initial(pcb_all_obs, yr = 2014:2018, matrix = "both")
```
...
```{r investigate approaches to qflags and summarizing by region vs basin pcb biota indicator, fig.width = 9.5, fig.height = 11}
...
**PFOS, Visualize flagged data in Timeseries**

Below the data are plotted in congener-subbasin combinations. Colorscale corresponds to number of observations, with lighter blues/greens indicating more observations for the given date-congener-subbasin group, while darker colors indicate fewer observations in the dataset for the given date-congener-subbasin combination.

```{r pfos qflag adjustment congeners by basin, results = "show", fig.height = 24, fig.width = 9.5}
## pcb qflag adjustment congeners by basin and congener, 
## red x's indicate flagged data, colorscale indicates number of observations
plotdf <- pfos %>% 
  mutate(value_adj = ifelse(qflagged, value_wet_wgt/2, NA)) %>% 
...
cols <- colorRampPalette(cols)(42)[sample(1:42, size = 19)]
plot_cwcon_initial(pfos_qflag_adjust, col_pal = cols, contam_param = "O-FL", cntry_or_basin = "country")
map_cwcon_initial(pfos_qflag_adjust %>% mutate(Matrix = "biota"), yr = 2014:2019, matrix = "biota")
```
