...
```{r save dioxin contaminants layer and intermediate datasets}
...
```{rconcerning substances monitoring in biota, fig.width = 9.5, fig.height = 15}
...
```{r concerning substances monitoring in sediments, fig.width = 9.5, fig.height = 15}
gridExtra::grid.arrange(grobs = plotlistbio, nrow = length(plotlistbio))
```
...
concern_subst_indicator <- concern_subst_indicator %>%
  ...
  mutate(num_substances = num_substances) %>%
  mutate(num_monitored = rowSums(.[grep("_monitored", names(.))])) %>% 
  mutate(proportion_monitored = round(num_monitored/num_substances, 2)) %>% 
