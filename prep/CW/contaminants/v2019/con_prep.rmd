...
pcb_sed <- pcb_sed %>% 
  ...
  select(
    variable, value, detect_lim, quant_lim,
...
plotdf <- pcb_sed %>% 
  mutate(value_adj = ifelse(qflagged, value/2, NA)) %>% 
  group_by(date, variable, Subbasin) %>% 
  mutate(countObs = n(), meanAdjww = mean(value_adj, na.rm = TRUE)) %>% 
  ungroup()
...
ggplot(plotdf) + 
  scale_color_distiller(palette = "GnBu", direction = -1) +
  geom_point(aes(date, value, color = countObs), show.legend = FALSE, size = 0.6, alpha = 0.4) +
  geom_point(aes(date, value_adj), color = "red", shape = 4, size = 1, alpha = 0.6) +
  facet_wrap(
    c("variable", "Subbasin"), 
    labeller = label_wrap_gen(width = 35, multi_line = FALSE), 
...
qflag_adjust <- function(dataset, congeners = NULL, matrix = "bio", sumcongeners = TRUE, approach = "Adjusted"){
  ...
  if(matrix == "bio"){
    ...
    dataset <- dataset %>% rename(value_plot = value)
  }
...
pcb_all_obs <- bind_rows(
  pcb_bio_qflag_adjust %>% 
    select(Subbasin, HELCOM_ID, year, latitude, longitude) %>% 
    mutate(Matrix = "biota"), 
...
## check: nrow(dioxin_teq) == nrow(dioxin_bio_sf)
