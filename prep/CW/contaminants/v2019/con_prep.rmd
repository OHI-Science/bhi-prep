...
ggplot(plotdf) + 
  ...
  geom_point(aes(date, value_adj, color = countObs), show.legend = FALSE, size = 0.6, alpha = 0.4) +
...
plotdf <- pcb_sed %>% 
  mutate(value_adj = ifelse(qflagged, value_dry_wgt/2, value_dry_wgt)) %>% 
  mutate(value_adj_original = ifelse(qflagged, value_dry_wgt, NA)) %>% 
  group_by(date, variable, Subbasin) %>% 
  mutate(countObs = n(), meanAdjww = mean(value_adj, na.rm = TRUE)) %>% 
  ungroup()
...
ggplot(plotdf) + 
  scale_color_distiller(palette = "GnBu", direction = -1) +
  geom_point(aes(date, value_adj, color = countObs), show.legend = FALSE, size = 0.8, alpha = 0.7) +
  geom_point(aes(date, value_adj_original), color = "red", shape = 4, size = 1, alpha = 0.6) +
  facet_wrap(
    c("variable", "Subbasin"), 
    labeller = label_wrap_gen(width = 35, multi_line = FALSE), 
...
map_cwcon_initial <- function(dataset, yr, matrix = NULL){
  ...
  obs_count_limits <- c(0, signif(max(df$ObservCount, na.rm = TRUE), 1))
  ...
  map <- ggplot(df) + 
    ...
    scale_fill_gradient(low = "midnightblue", high = "lightcyan", na.value = "whitesmoke", limits = obs_count_limits) +
    geom_sf(
  ...
  basinplot <- ggplot(filter(df, !is.na(Matrix))) +
    geom_point(
      aes(Subbasin, meanVal, fill = matrix_ObservCount, size = numLocation),
      color = "burlywood", shape =  21
    ) +
    labs(x = NULL, y = "Mean concentration (ug/kg)", color = NULL) +
    scale_fill_gradient(low = "midnightblue", high = "lightcyan", na.value = "whitesmoke", limits = obs_count_limits) +
...
```{r map spatial dist of PCBs year 2014, results = "show", fig.width = 9.5, out.width = "50%"}
pcb2014 <- map_cwcon_initial(pcb_qflag_adjust_datemeans, yr = 2014, matrix = "both")
...
```{r map spatial dist of PCBs year 2016, results = "show", fig.width = 9.5, out.width = "50%"}
pcb2016 <- map_cwcon_initial(pcb_qflag_adjust_datemeans, yr = 2016, matrix = "both")
...
```{r map spatial dist of PCBs multiple years, results = "show", fig.width = 9.5, out.width = "50%"}
pcb2014to2019 <- map_cwcon_initial(pcb_qflag_adjust_datemeans, yr = 2014, matrix = "both")
...
```{r investigate approaches to qflags and summarizing by region vs basin pcb biota indicator, fig.width = 9.5, fig.height = 7}
...
pcb_qflag_adjust_indicators <- cwcon_indicators(pcb_qflag_adjust_datemeans)
pcb_noqflag_indicators <- cwcon_indicators(pcb_noqflag_datemeans)
...
```{r pfos qflag adjustment congeners by basin, results = "show", fig.width  = 9.5, fig.height = 9}
## pcb qflag adjustment congeners by basin and congener, 
## red x's indicate flagged data, colorscale indicates number of observations
plotdf <- pfos %>% 
  mutate(value_adj = ifelse(qflagged, value_wet_wgt/2, value_wet_wgt)) %>% 
...
ggplot(plotdf) + 
  ...
  geom_point(aes(date, value_adj, color = countObs), show.legend = FALSE, size = 0.8, alpha = 0.9) +
...
###### Boxplot, Concentrations (muscle equivalent) by Year
...
ggplot(boxplotdf) + 
  ...
  geom_boxplot(aes(year, meanAdjww, color = countObs), show.legend = FALSE, outlier.size = 0.3) + 
...
```{r pfos map of data records, results = "show", fig.width = 9.5, out.width = "50%"}
...
pfos2008to2019 <- map_cwcon_initial(pfos_qflag_adjust_datemeans, yr = 2008:2019)
...
## (1) adjusting vs excluding flagged data, and 
## (2) aggregating data by subbasin versus BHI regions in calculating status and trend
pfos_qflag_adjust_indicators <- cwcon_indicators(pfos_qflag_adjust_datemeans, bio_thresh = 9.1)
pfos_noqflag_indicators <- cwcon_indicators(pfos_noqflag_datemeans, bio_thresh = 9.1)
