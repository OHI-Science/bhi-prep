...
These are saved to the `layers/v2019` folder. Intermediate datasets saved to `data/CW/contaminants/v2019/intermediate` include: `pcb_bio_cleaned.csv`, `pcb_sed_cleaned.csv`, `pfos_bio_cleaned.csv`, `dioxin_bio_cleaned.csv` and `dioxin_sed_cleaned.csv`. All these are derived from or informed by the following raw datasets.
...
```{r save pcbs contaminants layer and any intermediate datasets}
write_csv(
  pcb_qflag_adjust_datemeans %>% 
    select(
      region_id = BHI_ID,
  ...
  file.path(dir_layers, version_year, sprintf("cw_con_pcb_bhi%s.csv", assess_year))
)
```
...
```{r save pfos contaminants layer and any intermediate datasets}
write_csv(
  pfos_qflag_adjust_datemeans %>% 
    select(
      region_id = BHI_ID,
  ...
  file.path(dir_layers, version_year, sprintf("cw_con_pfos_bhi%s.csv", assess_year))
)
```
...
**Save Dioxin contaminants layer and intermediate datasets**

```{r save dioxin contaminants layer and any intermediate datasets}
write_csv(
  dioxins_qflag_adjust_teqs_total %>% 
    select(
      region_id = BHI_ID,
  ...
  file.path(dir_layers, version_year, sprintf("cw_con_dioxin_bhi%s.csv", assess_year))
)
```
...
concern_subst_indicator <- tidyr::pivot_wider(
  rgn_monitor_allsubst,
  names_from = "substance", 
  values_from = "monitored"
)
yrs <- 2014:2018
...
concern_subst_indicator <- concern_subst_indicator %>%
  ...
  select(rgn_nam, rgn_key, Subbasin, HELCOM_ID, BHI_ID, proportion_monitored)
```

<br>

**Save concerning substances layer and intermediate datasets**

```{r save oncerning substances layer and intermediate datasets}
write_csv(
  rgn_monitor_allsubst %>% 
    select(
      region_id = BHI_ID,
  ...
  file.path(dir_layers, version_year, sprintf("cw_con_penalty_bhi%s.csv", assess_year))
)
```
