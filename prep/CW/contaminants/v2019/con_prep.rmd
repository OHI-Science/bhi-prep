...
Same as plots above, but with fewer data points. Higher opacity and larger points used just so they are easier to see in the plots.

```{r pcbs sed qflag adjustment congeners by basin, results = "show", fig.height = 24, fig.width = 9.5}
plotdf <- pcb_sed %>% 
...
plot_cwcon_initial(pcb_sed_qflag_adjust, col_pal = cols, contam_param = "OC-CB", matrix = "sed", cntry_or_basin = "country")
```
...
```{r map spatial dist of PCB bio one year, results = "show", echo = FALSE, fig.width = 10, fig.height = 9, out.width = "120%"}
map_cwcon_initial(pcb_all_obs, yr = 2014, matrix = "both")
```

##### With years 2016-2019

```{r map spatial dist of PCB bio multiple yrs, results = "show", echo = FALSE, fig.width = 10, fig.height = 9, out.width = "120%"}
map_cwcon_initial(pcb_all_obs, yr = 2016:2019, matrix = "both")
```
---
...
```{r investigate approaches to qflags and summarizing by region vs basin pcb biota indicator, fig.width = 9.5, fig.height = 17}
...
```{r pfos qflag adjustment congeners by basin, results = "show", fig.height = 28, fig.width = 9.5}
## pcb qflag adjustment congeners by basin and congener, 
## red x's indicate flagged data, colorscale indicates number of observations
plotdf <- pfos %>% 
  mutate(value_adj = ifelse(qflagged, value_wet_wgt/2, NA)) %>% 
