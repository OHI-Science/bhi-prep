...
```{r map spatial dist of PCBs year 2014, results = "show", fig.width = 13, fig.height = 7.5, out.width = "120%"}
pcb2014 <- map_cwcon_initial(pcb_qflag_adjust_datemeans, yr = 2014, c(0, 15), matrix = "both")
...
```{r map spatial dist of PCBs year 2016, results = "show", fig.width = 13, fig.height = 7.5, out.width = "120%"}
pcb2016 <- map_cwcon_initial(pcb_qflag_adjust_datemeans, yr = 2016, c(0, 15), matrix = "both")
...
get_spatial_monitor_info <- function(substance_data_filepath, matrix = "bio", yrs, bhi_join_tab){
  ...
  countdfplot <- ggplot(countdf %>% mutate(BHI_ID = as.factor(BHI_ID))) + 
    geom_point(
      aes(BHI_ID, year, color = count_obs), 
      show.legend = FALSE,
      size = 5, 
      shape = 15, 
      alpha = 0.7
    ...
    labs(x = NULL, y = NULL, color = "Num. Obs.") +
  ...
  monitordf <- countdf %>% 
    ...
    # filter(count_obs > 2) %>% 
    select(BHI_ID, HELCOM_ID, Subbasin, rgn_nam = country, ICES_monitored) %>%
    distinct() %>% 
    right_join(bhi_join_tab) %>% 
...
```{rconcerning substances monitoring in biota, fig.width = 9.5, fig.height = 14}
...
```{r concerning substances monitoring in sediments, fig.width = 9.5, fig.height = 14}
gridExtra::grid.arrange(grobs = plotlistbio, nrow = length(plotlistbio))
```
...
```{r calculating emerging concerning substances indicator}
...
## for the remaining substances of concern (not monitored at all) add columns
for(s in unique(filter(subst_of_concern, !ICES_monitored)$filename)){
  df <- mutate(rgn_monitor_allsubst, addcolumn = FALSE)
  colnames(df) <- c(names(rgn_monitor_allsubst), paste0(s, "_monitored"))
  rgn_monitor_allsubst <- df
}
num_substances <- length(grep("_monitored", colnames(rgn_monitor_allsubst)))
rgn_monitor_allsubst <- rgn_monitor_allsubst %>%
  mutate(num_substances = num_substances_bio) %>%
  mutate(num_monitored = rowSums(.[grep("_monitored", names(.))]))
```
...
```{r save pfos contaminants layer and intermediate datasets}
write_csv(rgn_monitor_allsubst, file.path(dir_layers, version_year, "cw_con_penalty.csv"))
```
