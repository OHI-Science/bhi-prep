...
read_clean_df <- function(filename){
  read_csv(
    ...
    col_types = cols(
      ...
      uncert_val = col_number(),
      method_uncert = col_character(),
      `LIPIDWT%` = col_number(),
      `DRYWT%` = col_number(),
      `EXLIP%` = col_number(),
      WTMEA = col_number(),
  ...
  ) %>% assign(x = str_replace(filename, "cleaned.csv", "clean_df"), envir = .GlobalEnv)
}
```
...
```{r what do the qflags indicate, results = "show", fig.height = 4, fig.width = 9.5}
## what do the qflags actually indicate?
chk_pcb_bio_qflags <- pcb_bio_sf %>% 
...
ggplot(chk_pcb_bio_qflags) +
  ...
  labs(
    x = "\nConc. value ≤ Quantification Limit ('<' and 'Q') or < Detection Limit ('D')", 
    y = NULL, 
    fill = NULL
  )  +
  theme_dark()
...
```{r qflag adjustment, results = "show", fig.height = 12, fig.width = 9.5}
ggplot(
  mutate(
    pcb_bio_sf, 
...
```{r pcb bio congeners by basin, results = "show"}
qflag_plot <- plotly::ggplotly(
  ggplot(filter(pcb_bio_sf, !is.na(value_wet_wgt)), aes(x = date, y = value_wet_wgt)) +
    geom_point(aes(color = qflagged, shape = basis_determination), alpha = 0.2, size = 1.2) +
    scale_color_manual(values = c("steelblue", "firebrick")) +
    facet_grid(vars(variable), vars(Subbasin), scales = "free_y") + 
    theme(axis.text.x = element_text(angle = 90)) +
    labs(x = NULL, y = "Concentration (ug/kg, Wet Weight)\n")
)
qflag_plot$x$layout$showlegend <- FALSE
qflag_plot
```
...
viz_cwcon_initial <- function(dataset, bhi_rgn_shp, plottype = c("map", "plot", "widget"), yr){
  ...
  ## make plotly widget with congener concentration values by subbasin
  if("widget" %in% plottype){
    congener_widget <- plotly::ggplotly(
      ggplot(filter(dataset, !is.na(value_wet_wgt)), aes(x = date, y = value_wet_wgt)) +
        geom_point(aes(color = qflagged, shape = basis_determination), alpha = 0.2, size = 1.2) +
        scale_color_manual(values = c("steelblue", "firebrick")) +
        facet_grid(vars(variable), vars(Subbasin), scales = "free_y") + 
        theme(axis.text.x = element_text(angle = 90)) +
    ...
    congener_widget$x$layout$showlegend <- FALSE
    result[["widget"]] <- congener_widget
  }
  return(result)
}
```
...
eval_cwcon_indicators <- function(datalayer, yrs = 2010:2019, threshold = 75, 
  ...
  id_obs_unique <- c(
    "BHI_ID", "Subbasin", "HELCOM_ID", "country",
    "date", "monit_year", "year", "month", "day", 
    "station", "report_institute", 
    "monit_program", "monit_purpose", 
    "sub_samp_ref", "sub_samp_id", "samp_ref", "num_indiv_subsample", "bulk_id"
  )
  
  ## filter where BHI regions have no data
  datalayer <- filter(datalayer, !is.na(variable))
  
  if(approach == "NoQflag"){
    datalayer <- filter(datalayer, !qflagged)
  }
  
  ## adjust for qflags 
  ## sum congeners by date and location
  ## remove cases where not all congeners are observed
  data_Qadjust_means <- datalayer %>% 
    mutate(value_adj = ifelse(qflagged, value_wet_wgt/2, value_wet_wgt)) %>% 
    select(!!!syms(id_obs_unique), variable, value_adj) %>% 
    cbind(st_coordinates(.)) %>% 
    st_drop_geometry() %>% 
    tidyr::pivot_wider(
      names_from = variable, 
      values_from = value_adj, 
      values_fn = list() # summarize duplicates (means!!!)........ but... should not be duplicates, shouldve already been dealt with....
    ) %>% 
  ...
  result[["basin_status"]] <- data_Qadjust_means %>% 
    filter() %>% 
    group_by() %>% 
    summarize(
      meanVal = , # mean value....
  ...
  basin_status <- data_Qadjust_means %>% 
  ...
  result[["bhirgn_trend"]] <- data_Qadjust_means %>% 
    mutate(obs = ) %>% 
    filter(year %in% yrs) %>%
    group_by(Subbasin) %>% 
