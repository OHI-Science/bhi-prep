...
```{r pcbs bio qflag adjustment congeners by basin, results = "show", fig.height = 19.5, fig.width = 9.5}
...
```{r pcbs sed qflag adjustment congeners by basin, results = "show", fig.height = 30, fig.width = 9.5}
...
plot_cwcon_initial <- function(dataset, col_pal, contam_param = "OC-CB", matrix = "bio", cntry_or_basin = "subbasin"){
  ...
  return(plot) # plotly::ggplotly(plot)
}
```
...
```{r map spatial dist of PCBs year 2014, results = "show", fig.width = 14, fig.height = 7.5, out.width = "120%"}
pcb2014 <- map_cwcon_initial(pcb_qflag_adjust_datemeans, c(0, 15), yr = 2014, matrix = "both")
...
```{r map spatial dist of PCBs year 2016, results = "show", fig.width = 14, fig.height = 7.5, out.width = "120%"}
pcb2016 <- map_cwcon_initial(pcb_qflag_adjust_datemeans, c(0, 15), yr = 2016, matrix = "both")
...
###### With years 2014 to 2018

```{r map spatial dist of PCBs multiple years, results = "show", fig.width = 14, fig.height = 7.5, out.width = "120%"}
pcb2014to2019 <- map_cwcon_initial(pcb_qflag_adjust_datemeans, c(0, 15), yr = 2014:2018, matrix = "both")
...
cwcon_indicators <- function(dat, yrs, bio_thresh = 75, sed_thresh = 4.1, approach = "pctChange", trendlag = 5){
  ...
  for(zn in c("basin", "bhirgn")){
    ...
    if(approach == "scaledObs"){
      ...
      result[[lstname]] <- dat %>% 
        ...
        ## OPTION 2
        # mutate(trendcalc_nyrs = n_distinct(year)) %>% 
        # do(trend_mdl = lm(sum_congeners_datemean ~ year, data = .)) %>%
        # mutate(
        #   health_threshold = ifelse(Matrix == "bio", bio_thresh, sed_thresh),
        #   projected = predict(trend_mdl, data.frame(year = max(yrs)+trendlag)),
        #   ratio = projected/health_threshold,
        #   projected_sum_congeners_datemean_scaled = pmin(1, 1/ifelse(ratio < 0, 1, ratio))
        # ) %>% 
        ## OPTIONS 1 & 2
        ungroup() %>% 
        select(-trend_mdl) %>% 
        ...
        # summarize(trend = mean(projected_sum_congeners_datemean_scaled, na.rm = TRUE)) %>%
    ...
    if(approach == "pctChange"){
      ...
      ## plot to check...
      # ggplot(left_join(filter(dat, year %in% trendyrs), select(lm_estim, Subbasin, projected, current_estimated))) +
      #   geom_point(aes(x = year, y = sum_congeners_datemean)) +
      #   geom_vline(xintercept = max(trendyrs)) +
      #   geom_vline(xintercept = max(trendyrs)+trendlag) +
      #   geom_hline(aes(yintercept = projected), color = "blue") +
      #   geom_hline(aes(yintercept = current_estimated)) +
      #   facet_wrap(~Subbasin, scales = "free") +
      #   labs(x = NULL, y = "Congners Sum, means by Date and Location")
    }
  }
...
pcb_qflag_adjust_indicators <- cwcon_indicators(pcb_qflag_adjust_datemeans, yrs = 2014:2019)
pcb_noqflag_indicators <- cwcon_indicators(pcb_noqflag_datemeans, yrs = 2014:2019)
...
**Save PCB contaminants layer and intermediate datasets**

```{r save pcbs contaminants layer and intermediate datasets}
...
```{r pfos qflag adjustment values by basin, results = "show", fig.width  = 9.5, fig.height = 7}
## pcb qflag adjustment congeners by basin and congener, 
## red x's indicate flagged data, colorscale indicates number of observations
plotdf <- pfos %>% 
...
###### Concentrations Boxplots by Year

```{r pfos qflag adjusted values basin boxplots, results = "show", fig.width  = 9.5, fig.height = 7}
boxplotdf <- plotdf %>%
...
```{r pfos map of data records, results = "show", fig.width = 14, fig.height = 7.5, out.width = "120%"}
...
```{r investigate approaches to qflags and summarizing by region vs basin pfos indicator, fig.width = 9.5, fig.height = 7}
## using function defined above, investigate:
## (1) adjusting vs excluding flagged data, and 
## (2) aggregating data by subbasin versus BHI regions in calculating status and trend
...
**Save PFOS contaminants layer and intermediate datasets**

```{r save pfos contaminants layer and intermediate datasets}
...
```{r checking dioxin sediment data and outliers, results = "show", fig.width = 10, fig.height = 5.7}
...
```{r dioxinlike pcb bio qflag adjustment congeners by basin, results = "show", fig.width = 9.5, fig.height = 18}
...
```{r dioxinlike pcb sed qflag adjustment congeners by basin, results = "show", fig.width = 9.5, fig.height = 20.7}
...
```{r dioxin bio qflag adjustment congeners by basin, results = "show", fig.width = 9.5, fig.height = 38.7}
...
```{r dioxin sed qflag adjustment congeners by basin, results = "show", fig.width = 9.5, fig.height = 45.9}
...
```{r map spatial dist of dioxinlike pcbs, results = "show", fig.width = 14, fig.height = 7.5, out.width = "120%"}
...
```{r map spatial dist of dioxins, results = "show", fig.width = 14, fig.height = 7.5, out.width = "120%"}
...
get_spatial_monitor_info <- function(substance_name, data_loc, matrix = "bio"){
  ...
  df <- df %>% 
    group_by(country, year, lat, lon, param_group, variable) %>% 
    summarize(count_obs = n()) %>% 
    ## should we filter, if fewer than a certain number of measurements in the region?
    # filter(count_obs > threshold) %>% 
    ungroup() %>% 
    join_rgns_info() %>% 
    filter(!is.na(BHI_ID)) %>% 
    select(year, count_obs, BHI_ID, HELCOM_ID, Subbasin, Country)
  
  return(df)
}

## read in master table of emerging contaminant
con_emerging <- read_csv(file.path(dir_B, ...)))

## loop through substances and determine which regions they are/not monitored in

for(nm in unique(con_emerging$substancescolumn)){
  regionalmonitoring <- get_spatial_monitor_info(nm)
