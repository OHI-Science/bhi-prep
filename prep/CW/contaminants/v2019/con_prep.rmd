...
```{r checking data and outliers, results = "show", fig.width = 9.5, fig.height = 8}
...
makeplot <- function(data){
  ggplot(data) + 
    geom_point(aes(date, value_adj), show.legend = FALSE, size = 0.2, alpha = 0.5, color =  "grey") +
    facet_wrap(~variable, scales = "free_y", nrow = 7) +
    labs(x = NULL, y = NULL)
}
gridExtra::grid.arrange(
  makeplot(plotdf) + 
    geom_point(data = filter(plotdf, !chk), aes(date, value_adj)) +
    ggtitle("All Data"),
  makeplot(filter(plotdf, !chk1)) + 
    geom_point(data = filter(plotdf, !chk1 & !chk), aes(date, value_adj)) +
    ggtitle("W/O station 473 July2015"),
  makeplot(filter(plotdf, !chk1 & !chk2)) + 
    geom_point(data = filter(plotdf, !chk1 & !chk2 & !chk), aes(date, value_adj)) +
...
1. Join dioxins and dioxin-like PCBs by `sub_sample_ref`, `date`, `latitude`, `longitude`
2. Get TEQ concentration for each sub_sample_ref
...
<!-- **TEQ concentration for each sub_sample_ref** -->
...
dioxinlikepcb_qflag_adjust_datemeans <- bind_rows(
  dioxinlike_pcbbio_qflag_adjust %>% 
    group_by(date, day, month, year, latitude, longitude, country, Subbasin, HELCOM_ID, BHI_ID) %>% 
    summarize(sum_congeners_datemean = mean(sum_congeners, na.rm = TRUE)) %>% 
    mutate(Matrix = "bio"),
  dioxinlike_pcbsed_qflag_adjust %>% 
    group_by(date, day, month, year, latitude, longitude, country, Subbasin, HELCOM_ID, BHI_ID) %>% 
    summarize(sum_congeners_datemean = mean(sum_congeners, na.rm = TRUE)) %>% 
    mutate(Matrix = "sed")
) %>% ungroup()

dioxinlikepcb_noqflag_datemeans <- bind_rows(
  dioxinlike_pcbbio_noqflag %>% 
    group_by(date, day, month, year, latitude, longitude, country, Subbasin, HELCOM_ID, BHI_ID) %>% 
    summarize(sum_congeners_datemean = mean(sum_congeners, na.rm = TRUE)) %>% 
    mutate(Matrix = "bio"),
  dioxinlike_pcbsed_noqflag %>% 
    group_by(date, day, month, year, latitude, longitude, country, Subbasin, HELCOM_ID, BHI_ID) %>% 
    summarize(sum_congeners_datemean = mean(sum_congeners, na.rm = TRUE)) %>% 
    mutate(Matrix = "sed")
) %>% ungroup()

dioxin_qflag_adjust_datemeans <- bind_rows(
  dioxinbio_qflag_adjust %>% 
    group_by(date, day, month, year, latitude, longitude, country, Subbasin, HELCOM_ID, BHI_ID) %>% 
    summarize(sum_congeners_datemean = mean(sum_congeners, na.rm = TRUE)) %>% 
    mutate(Matrix = "bio"),
  dioxinsed_qflag_adjust %>% 
    group_by(date, day, month, year, latitude, longitude, country, Subbasin, HELCOM_ID, BHI_ID) %>% 
    summarize(sum_congeners_datemean = mean(sum_congeners, na.rm = TRUE)) %>% 
    mutate(Matrix = "sed")
) %>% ungroup()

dioxin_noqflag_datemeans <- bind_rows(
  dioxinbio_noqflag %>% 
    group_by(date, day, month, year, latitude, longitude, country, Subbasin, HELCOM_ID, BHI_ID) %>% 
    summarize(sum_congeners_datemean = mean(sum_congeners, na.rm = TRUE)) %>% 
    mutate(Matrix = "bio"),
...
**Sum dioxin and dioxin-like PCB mean TEQ values by date and location**
