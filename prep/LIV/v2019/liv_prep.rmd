...
```{r read in datasets with liv employment by country}
...
```{r merge with bhi regions save data layers for the liv subgoal}
...
**Calculating the LIV Status**

```{r calculate liv status}
...
```{r plot of liv status, fig.width = 9.5, results = "show"}
gridExtra::grid.arrange(
  ggplot(liv_status) + 
    ...
    facet_wrap(~region_name, labeller = label_wrap_gen(width = 15)) +
    labs(x = NULL, y = NULL, fill = "Percentage", title = "Blue Econ. Empl. % of National Empl.") +
    theme(legend.direction = "horizontal", legend.position = c(0.85, 0.08)),
  ggplot(liv_status) + 
...
Consider: should employment in marine-related sectors be normalize just by coastal vs total national population, or is potential for coastal jobs more related to geography (e.g. length coastline vs total area)?
